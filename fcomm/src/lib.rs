...
pub fn nova_proof_cache(reduction_count: usize) -> NovaProofCache {
  FileMap::<Cid, Proof<S1>>::new(format!("nova_proofs.{}", reduction_count))
    .unwrap()
}

pub type CommittedExpressionMap =
  FileMap<Commitment<S1>, CommittedExpression<S1>>;
pub fn committed_expression_store() -> CommittedExpressionMap {
  FileMap::<Commitment<S1>, CommittedExpression<S1>>::new(
    "committed_expressions",
  )
  .unwrap()
}

pub type PublicParamMemCache =
  Mutex<HashMap<usize, Arc<PublicParams<'static>>>>;
...
impl LurkPtr {
  ...
  pub fn from_ptr<F: LurkField + Serialize>(
    s: &mut Store<F>,
    ptr: &Ptr<F>,
  ) -> Self {
...
impl<'a> Proof<'a, S1> {
  ...
  pub fn verify(&self, pp: &PublicParams) -> Result<VerificationResult, Error> {
    ...
    let claim_iterations_and_num_steps_are_consistent =
      if let Claim::Evaluation(Evaluation {
  ...
  pub fn evaluation_io(
    ...
    let output_io = {
      ...
      let cont = evaluation.status.to_cont(s).ok_or_else(|| {
  ...
  pub fn opening_io(
    ...
    let input_io =
      IO::<S1> { expr: expression, env: empty_sym_env(s), cont: outermost };
...
mod test {
  ...
  fn test_cert_serialization() {
    ...
    let c = Commitment { comm: S1::from(123) };
