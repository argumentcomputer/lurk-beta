...
pub fn committed_function_store(
) -> FileMap<Commitment<Scalar>, Function<Scalar>> {
...
fn get_pvk(rc: ReductionCount) -> groth16::PreparedVerifyingKey<Bls12> {
  ...
  let public_params =
    Groth16Prover::create_groth_params(rc.reduction_frame_count()).unwrap();
...
impl<F: LurkField> Serialize for Commitment<F> {
  ...
  where S: Serializer {
    ...
    let be_bytes: Vec<u8> =
      self.comm.to_repr().as_ref().iter().rev().map(|x| x.to_owned()).collect();
...
impl<F: LurkField> FromHex for Commitment<F> {
  ...
  where T: AsRef<[u8]> {
    ...
    Ok(Commitment { comm: F::from_repr(repr).unwrap() })
  }
}
...
// This is just a rough idea, mostly here so we can plumb it elsewhere. The idea
// is that a verifier can sign an attestation that a given claim's proof was
// verified. It motivates the use of an online verifier for demo purposes.
// Although real proofs should be fast to verify, they will still be large
// relative to a small (auditable) bundle like this. Even if not entirely
// realistic, something with this general *shape* is likely to play a role in a
// recursive system where the ability to aggregate proof verification more
// soundly is possible.
...
impl<F: LurkField> Claim<F> {
  pub fn is_evaluation(&self) -> bool { self.evaluation().is_some() }

  pub fn is_opening(&self) -> bool { self.opening().is_some() }
...
impl From<serde_json::Error> for Error {
  fn from(err: serde_json::Error) -> Error { Error::JsonError(err) }
}
impl From<SynthesisError> for Error {
  fn from(err: SynthesisError) -> Error { Error::SynthesisError(err) }
}
...
where for<'de> T: Deserialize<'de>
{
  ...
  fn id(&self) -> String { self.cid().to_string() }

  fn has_id(&self, id: String) -> bool { self.id() == id }
}
...
where for<'de> T: Deserialize<'de> /* + Decode<DagJsonCodec>, */
{
  fn write_to_path<P: AsRef<Path>>(&self, path: P) {
...
impl Evaluation {
  fn new<F: LurkField>(
    ...
    input: IO<F>,
    output: IO<F>,
    iterations: Option<usize>, /* This might be padded, so is not quite
                                * 'iterations' in the sense of number of
                                * actual reduction steps required
                                * to evaluate. */
  ) -> Self {
    let status: Status = output.cont.into();
    let terminal = status.is_terminal();

    // For now, conservatively hide all outputs unless output is terminal. TODO:
    // let evaluator configure this in a more fine-grained way, including no
    // hiding. NOTE: If anything is hidden, a proof won't be possible.
    ...
    Self { expr, env, cont, expr_out, env_out, cont_out, status, iterations }
  }
  ...
  pub fn eval<F: LurkField + Serialize>(
    ...
    let (output, iterations, _) =
      evaluator.eval().map_err(|_| Error::EvaluationFailure)?;
  ...
  pub fn ptr<F: LurkField + Serialize + DeserializeOwned>(
    ...
    match self {
      ...
      LurkPtr::ScalarBytes(lurk_scalar_bytes) => {
        ...
        let lurk_ptr =
          LurkPtr::Ipld(LurkScalarIpld { scalar_store, scalar_ptr });
...
impl Opening<Scalar> {
  ...
  pub fn open_and_prove(
    ...
    let function =
      function_map.get(commitment).ok_or(Error::UnknownCommitment)?;
  ...
  fn _is_chained(&self) -> bool { self.new_commitment.is_some() }
  ...
  pub fn apply(
    ...
    let (new_commitment, output_expr) = if chain {
      ...
      let new_secret =
        *s.get_expr_hash(&new_secret0).expect("hash missing").value();
    ...
    let output_string = if status.is_terminal() {
      ...
      // We don't want to leak any internal information in the case of
      // incomplete computations. Provers might want to expose results in
      // the case of explicit errors. For now, don't -- but consider
...
impl Proof<Bls12> {
  ...
  pub fn prove_claim(
    ...
    info!("Getting Parameters");
    let public_params = Groth16Prover::create_groth_params(
      reduction_count.reduction_frame_count(),
    )
    .unwrap();
    let groth_prover =
    ...
    let (expr, env) = match &claim {
      ...
      Claim::Opening(o) => {
        ...
        // In order to prove the opening, we need access to the original
        // function.
    ...
    let proof =
      Proof { claim: claim.clone(), reduction_count, proof: groth_proof };

    match &proof.claim {
      Claim::Opening(o) => {
        if o.status != Status::Terminal {
          return Err(Error::OpeningFailure(
            "Claim status is not Terminal".into(),
          ));
        };
      },
      Claim::Evaluation(e) => {
  ...
  pub fn verify_evaluation(&self) -> Result<(Vec<Scalar>, Vec<Scalar>), Error> {
    ...
    let evaluation =
      &self.claim.evaluation().expect("expected evaluation claim");
    ...
    let output_io = {
      let expr = s.read(&evaluation.expr_out).map_err(|_| {
        Error::VerificationError("failed to read expr out".into())
      })?;

      let env = s.read(&evaluation.env_out).map_err(|_| {
        Error::VerificationError("failed to read env out".into())
      })?;
      let cont = evaluation.status.to_cont(&mut s).ok_or_else(|| {
        Error::VerificationError("continuation cannot be proved".into())
      })?;
  ...
  pub fn verify_opening(&self) -> Result<(Vec<Scalar>, Vec<Scalar>), Error> {
    ...
    let expr =
      s.hash_expr(&expression).expect("failed to hash input expression");
...
impl Key<Cid> for Proof<Bls12> {
  fn key(&self) -> Cid { self.claim.cid() }
}

impl VerificationResult {
  fn new(verified: bool) -> Self { Self { verified } }
}
...
mod test {
  ...
  fn test_cert_serialization() {
    ...
    let c = Commitment { comm: Scalar::from(123) };
