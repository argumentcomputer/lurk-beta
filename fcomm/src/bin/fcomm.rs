...
use fcomm::{
  self, committed_expression_store, error::Error, evaluate, public_params,
  Claim, Commitment, CommittedExpression, Evaluation, Expression, FileStore,
  LurkPtr, Opening, OpeningRequest, Proof, ReductionCount, S1,
};
...
impl Open {
  fn open(&self, limit: usize, eval_input: bool) {
    ...
    let handle_claim =
      |claim: Claim<S1>| serde_json::to_writer(io::stdout(), &claim);
    ...
    if let Some(request_path) = &self.request {
      ...
      if let Some(out_path) = &self.proof {
        let proof =
          Opening::open_and_prove(s, request, limit, false, &prover, &pp)
            .unwrap();
        ...
        let claim =
          Opening::apply(s, input, function, limit, self.chain).unwrap();
      ...
      let input =
        input(s, input_path, eval_input, limit, self.quote_input).unwrap();
