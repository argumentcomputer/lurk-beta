...
fn fcomm_cmd() -> std::process::Command { Command::cargo_bin("fcomm").unwrap() }
...
fn test_bad_command() {
  ...
  cmd.assert().failure().stderr(predicate::str::contains(
    "error: Found argument 'uiop' which wasn't expected, or isn't valid in \
     this context",
  ));
}
...
fn test_eval_expression() {
  ...
  cmd.arg("eval").arg("--expression").arg(expression_path).arg("--lurk");

  cmd.assert().success().stdout(
    "{\"expr\":\"((LAMBDA (A B) (+ (* A 3) B)) 9 \
     7)\",\"env\":\"NIL\",\"cont\":\"Outermost\",\"expr_out\":\"34\",\"\
     env_out\":\"NIL\",\"cont_out\":\"Terminal\",\"status\":\"Terminal\",\"\
     iterations\":17}",
  );
}
...
fn test_verify_expression_proof<T: AsRef<OsStr>>(
  mut cmd: Command,
  proof_path: T,
  _data_path: T,
) {
...
fn test_verify_opening<T: AsRef<OsStr>>(
  mut cmd: Command,
  proof_path: T,
  _data_path: T,
) {
...
fn test_prove_and_verify_expression() {
...

  {
    ...
    assert_eq!(
      proof.claim.evaluation().expect("expected evaluation claim").expr_out,
      expected
    );
  }
...
fn test_function_aux(
  ...
  let mut commitment: Commitment<Scalar> =
    Commitment::read_from_path(&commitment_path).unwrap();
  ...
  for (function_input, expected_output) in io {
    ...
    let canonical_output =
      store.read(expected_output).unwrap().fmt_to_string(&store);
...
fn test_create_open_and_verify_chained_functional_commitment() {
  let function_source = "(letrec ((secret 12345) (a (lambda (acc x) (let \
                         ((acc (+ acc x))) (cons acc (hide secret (a \
                         acc))))))) (a 0))";

  let expected_io = vec![("5", "5"), ("3", "8")];

  test_create_open_and_verify_chained_functional_commitment_aux(
    function_source,
    expected_io,
  );
}
...
{
  let function_source =
    "(letrec ((secret-data '((joe 4 3) (bill 10 2 3) (jane 8 7 6 10) (carol 3 \
     5 8))) (filter (lambda (data predicate) (if data (if (predicate (cdr \
     (car data))) (cons (car data) (filter (cdr data) predicate)) (filter \
     (cdr data) predicate))))) (f (lambda (predicate) (car (car (filter \
     secret-data predicate)))))) f)";
