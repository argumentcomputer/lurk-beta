...
impl<E: Tag, F: LurkField> PartialOrd for SPtr<E, F> {
  fn partial_cmp(&self, other: &Self) -> Option<core::cmp::Ordering> {
    (self.0.to_field_bytes::<F>().as_ref(), self.1.to_repr().as_ref())
      .partial_cmp(&(
...
impl<E: Tag, F: LurkField> Encodable for SPtr<E, F> {
  fn ser(&self) -> LightData {
    let (x, y): (FWrap<F>, FWrap<F>) =
      (FWrap(self.0.to_field()), FWrap(self.1));
    (x, y).ser()
  }
  fn de(ld: &LightData) -> anyhow::Result<Self> {
    let (x, y): (FWrap<F>, FWrap<F>) = Encodable::de(ld)?;
    let tag_as_u16 = x.0.to_u16().ok_or_else(|| {
      anyhow!("invalid range for field element representing a tag")
    })?;
...
impl<E: Tag, F: LurkField> Serialize for SPtr<E, F> {
    ...
    S: serde::Serializer,
  {
    ...
    let hash = Multihash::wrap(codec, &val.to_bytes()).map_err(|_| {
...
impl<'de, E: Tag, F: LurkField> Deserialize<'de> for SPtr<E, F> {
    ...
    D: serde::Deserializer<'de>,
  {
    ...
    let val = F::from_bytes(cid.hash().digest()).ok_or_else(|| {
      D::Error::custom("expected ScalarContPtr value".to_string())
    })?;
    // TODO(fga): eliminate this round-trip through the field
    let e_tag =
      E::from_field(&tag).ok_or_else(|| D::Error::custom("invalid Tag"))?;
...
impl<F: LurkField> Store<F> {
  ...
  pub fn intern_scalar_ptr(
    ...
    match (tag, expr) {
      ...
      (ExprTag::Num, Some(Num(x))) => {
        Some(self.intern_num(crate::Num::Scalar(*x)))
      },
      ...
      (ExprTag::Fun, Some(Fun { arg, body, closed_env })) => {
  ...
  pub fn scalar_from_parts(&self, tag: F, value: F) -> Option<ScalarPtr<F>> {
    ...
    self.scalar_ptr_map.contains_key(&scalar_ptr).then_some(scalar_ptr)
  }
  ...
  pub fn fetch(&self, ptr: &Ptr<F>) -> Option<Expression<F>> {
    ...
    match ptr.0 {
      ...
      ExprTag::Cons => {
        self.fetch_cons(ptr).map(|(a, b)| Expression::Cons(*a, *b))
      },
      ExprTag::Comm => {
        self.fetch_comm(ptr).map(|(a, b)| Expression::Comm(a.0, *b))
      },
...
impl<F: LurkField> ConstantPtrs<F> {
  ...
  pub fn scalar_ptr(&self) -> ScalarPtr<F> {
    self.0.expect(
      "ScalarPtr missing; hydrate_scalar_cache should have been called.",
    )
  }
