...
impl<F: LurkField> HashConstants<F> {
  ...
  pub fn constants(&self, arity: HashArity) -> HashConst<F> {
    match arity {
      HashArity::A3 => {
        HashConst::A3(self.c3.get_or_init(|| PoseidonConstants::new()))
      },
      HashArity::A4 => {
        HashConst::A4(self.c4.get_or_init(|| PoseidonConstants::new()))
      },
      HashArity::A6 => {
        HashConst::A6(self.c6.get_or_init(|| PoseidonConstants::new()))
      },
      HashArity::A8 => {
        HashConst::A8(self.c8.get_or_init(|| PoseidonConstants::new()))
      },
    }
  }
}
...
pub struct Store<F: LurkField> {
  ...
  pub(crate) scalar_ptr_map:
    dashmap::DashMap<ScalarPtr<F>, Ptr<F>, ahash::RandomState>,
  /// Holds a mapping of ScalarPtr -> ContPtr<F> for reverse lookups
  scalar_ptr_cont_map:
    dashmap::DashMap<ScalarContPtr<F>, ContPtr<F>, ahash::RandomState>,
...
pub trait Pointer<F: LurkField + From<u64>>:
  fmt::Debug + Copy + Clone + PartialEq + Hash
{
  ...
  fn tag_field(&self) -> F { F::from(self.tag().into()) }
}

pub trait ScalarPointer<F: LurkField>:
  fmt::Debug + Copy + Clone + PartialEq + Hash
{
...
impl<F: LurkField> Ptr<F> {
  ...
  pub fn is_fun(&self) -> bool { matches!(self.0, Tag::Fun) }

  pub fn is_opaque(&self) -> bool { self.1.is_opaque() }
}
...
impl<F: LurkField> Serialize for ScalarPtr<F> {
  ...
  where S: serde::Serializer {
    use ser::Error;
    let cid = F::to_cid(self.0, self.1).ok_or_else(|| {
      S::Error::custom("expected validly tagged ScalarPtr".to_string())
    })?;
...
impl<F: LurkField> ScalarPointer<F> for ScalarPtr<F> {
  fn from_parts(tag: F, value: F) -> Self { ScalarPtr(tag, value) }

  fn tag(&self) -> &F { &self.0 }

  fn value(&self) -> &F { &self.1 }
}
...
impl<F: LurkField> IntoHashComponents<F> for [F; 2] {
  fn into_hash_components(self) -> [F; 2] { self }
}

impl<F: LurkField> IntoHashComponents<F> for ScalarPtr<F> {
  fn into_hash_components(self) -> [F; 2] { [self.0, self.1] }
}
...
impl<F: LurkField> Serialize for ScalarContPtr<F> {
  ...
  where S: serde::Serializer {
    use ser::Error;
    let cid = F::to_cid(self.0, self.1).ok_or_else(|| {
      S::Error::custom("expected validly tagged ScalarContPtr".to_string())
    })?;
    cid.serialize(serializer)
  }
}
  ...
  where D: serde::Deserializer<'de> {
    ...
    let (tag, dig) = F::from_cid(cid).ok_or_else(|| {
      D::Error::custom(format!(
        "expected ScalarContPtr encoded as Cid, got {cid}"
      ))
    })?;
...
impl<F: LurkField> ScalarPointer<F> for ScalarContPtr<F> {
  fn from_parts(tag: F, value: F) -> Self { ScalarContPtr(tag, value) }
...
impl<F: LurkField> Pointer<F> for ContPtr<F> {
  ...
  fn tag(&self) -> Self::Tag { self.0 }
}

impl<F: LurkField> ContPtr<F> {
  pub fn new(tag: ContTag, raw_ptr: RawPtr<F>) -> Self { Self(tag, raw_ptr) }

  pub fn is_error(&self) -> bool { matches!(self.0, ContTag::Error) }
}
...
impl<F: LurkField> RawPtr<F> {
  fn new(p: usize) -> Self { RawPtr((p, false), Default::default()) }

  fn is_opaque(&self) -> bool { self.0 .1 }

  pub fn idx(&self) -> usize { self.0 .0 }
}
...
impl<F: LurkField> Continuation<F> {
  ...
  pub fn insert_in_store(&self, store: &mut Store<F>) -> (usize, bool) {
    match self {
      ...
      Self::Call0 { saved_env, continuation } => {
        store.call0_store.insert_full((*saved_env, *continuation))
      },
      Self::Call { unevaled_arg, saved_env, continuation } => {
        store.call_store.insert_full((*unevaled_arg, *saved_env, *continuation))
      },
      Self::Call2 { function, saved_env, continuation } => {
        store.call2_store.insert_full((*function, *saved_env, *continuation))
      },
      Self::Tail { saved_env, continuation } => {
        store.tail_store.insert_full((*saved_env, *continuation))
      },
      Self::Lookup { saved_env, continuation } => {
        store.lookup_store.insert_full((*saved_env, *continuation))
      },
      Self::Unop { operator, continuation } => {
        store.unop_store.insert_full((*operator, *continuation))
      },
      Self::Binop { operator, saved_env, unevaled_args, continuation } => store
        .binop_store
        .insert_full((*operator, *saved_env, *unevaled_args, *continuation)),
      Self::Binop2 { operator, evaled_arg, continuation } => {
        store.binop2_store.insert_full((*operator, *evaled_arg, *continuation))
      },
      Self::If { unevaled_args, continuation } => {
        store.if_store.insert_full((*unevaled_args, *continuation))
      },
      Self::Let { var, body, saved_env, continuation } => {
        store.let_store.insert_full((*var, *body, *saved_env, *continuation))
      },
      Self::LetRec { var, body, saved_env, continuation } => {
        store.letrec_store.insert_full((*var, *body, *saved_env, *continuation))
      },
      Self::Emit { continuation } => {
        store.emit_store.insert_full(*continuation)
      },
    }
  }
  ...
  pub fn cont_tag(&self) -> ContTag {
    match self {
      ...
      Self::Call0 { saved_env: _, continuation: _ } => ContTag::Call0,
      Self::Call { unevaled_arg: _, saved_env: _, continuation: _ } => {
        ContTag::Call
      },
      Self::Call2 { function: _, saved_env: _, continuation: _ } => {
        ContTag::Call2
      },
      Self::Tail { saved_env: _, continuation: _ } => ContTag::Tail,
      Self::Lookup { saved_env: _, continuation: _ } => ContTag::Lookup,
      Self::Unop { operator: _, continuation: _ } => ContTag::Unop,
      Self::Binop {
        operator: _,
        saved_env: _,
        unevaled_args: _,
      ...
      Self::Binop2 { operator: _, evaled_arg: _, continuation: _ } => {
        ContTag::Binop2
      },
      Self::If { unevaled_args: _, continuation: _ } => ContTag::If,
      Self::Let { var: _, body: _, saved_env: _, continuation: _ } => {
        ContTag::Let
      },
      Self::LetRec { var: _, body: _, saved_env: _, continuation: _ } => {
        ContTag::LetRec
      },
...
#[derive(
  Debug, Copy, Clone, PartialEq, Eq, Hash, Serialize_repr, Deserialize_repr,
)]
#[repr(u16)]
pub enum Tag {
...
impl From<ContTag> for u64 {
  fn from(t: ContTag) -> Self { t as u64 }
}
...
/// manipulating Lurk data. They can be thought of as a minimal DSL for working
/// with Lurk data in Rust code. Prefer these methods when constructing literal
/// data or assembling program fragments in tests or during evaluation, etc.
impl<F: LurkField> Store<F> {
  pub fn nil(&mut self) -> Ptr<F> { self.intern_nil() }

  pub fn t(&mut self) -> Ptr<F> { self.lurk_sym("T") }
  ...
  pub fn list(&mut self, elts: &[Ptr<F>]) -> Ptr<F> { self.intern_list(elts) }

  pub fn num<T: Into<Num<F>>>(&mut self, num: T) -> Ptr<F> {
    self.intern_num(num)
  }

  pub fn uint64(&mut self, n: u64) -> Ptr<F> { self.get_u64(n) }
  ...
  pub fn intern_nil(&mut self) -> Ptr<F> { self.lurk_sym("nil") }
  ...
  pub fn intern_strcons(&mut self, car: Ptr<F>, cdr: Ptr<F>) -> Ptr<F> {
    ...
    let (c, s) =
      (self.fetch_char(&car).unwrap(), self.fetch_str(&cdr).unwrap());
  ...
  // Intern a potentially-opaque value. If the corresponding value is already
  // known to the store, return the known value.
  fn intern_maybe_opaque(&mut self, tag: Tag, hash: F) -> Ptr<F> {
    self.intern_opaque_aux(tag, hash, true)
  }

  // Intern an opaque value. If the corresponding non-opaque value is already
  // known to the store, return an opaque one anyway.
  ...
  // already known to the store, and `return_non_opaque_if_existing` is true,
  ...
  pub fn intern_scalar_cont_ptr(
    ...
    if let Some(cont) = scalar_store.get_cont(&ptr) {
      let continuation = match cont {
        ...
        Call { unevaled_arg, saved_env, continuation } => Continuation::Call {
          unevaled_arg: self.intern_scalar_ptr(*unevaled_arg, scalar_store)?,
          saved_env: self.intern_scalar_ptr(*saved_env, scalar_store)?,
          continuation: self
            .intern_scalar_cont_ptr(*continuation, scalar_store)?,
        },

        Call2 { function, saved_env, continuation } => Continuation::Call2 {
          function: self.intern_scalar_ptr(*function, scalar_store)?,
          saved_env: self.intern_scalar_ptr(*saved_env, scalar_store)?,
          continuation: self
            .intern_scalar_cont_ptr(*continuation, scalar_store)?,
        },
        Tail { saved_env, continuation } => Continuation::Tail {
          saved_env: self.intern_scalar_ptr(*saved_env, scalar_store)?,
          continuation: self
            .intern_scalar_cont_ptr(*continuation, scalar_store)?,
        },
        Error => Continuation::Error,
        Lookup { saved_env, continuation } => Continuation::Lookup {
          saved_env: self.intern_scalar_ptr(*saved_env, scalar_store)?,
          continuation: self
            .intern_scalar_cont_ptr(*continuation, scalar_store)?,
        },
        Unop { operator, continuation } => Continuation::Unop {
          operator: *operator,
          continuation: self
            .intern_scalar_cont_ptr(*continuation, scalar_store)?,
        },
        Binop { operator, saved_env, unevaled_args, continuation } => {
          Continuation::Binop {
            ...
            saved_env: self.intern_scalar_ptr(*saved_env, scalar_store)?,
            unevaled_args: self
              .intern_scalar_ptr(*unevaled_args, scalar_store)?,
            continuation: self
              .intern_scalar_cont_ptr(*continuation, scalar_store)?,
          }
        },
        Binop2 { operator, evaled_arg, continuation } => Continuation::Binop2 {
          operator: *operator,
          evaled_arg: self.intern_scalar_ptr(*evaled_arg, scalar_store)?,
          continuation: self
            .intern_scalar_cont_ptr(*continuation, scalar_store)?,
        },
        If { unevaled_args, continuation } => Continuation::If {
          unevaled_args: self
            .intern_scalar_ptr(*unevaled_args, scalar_store)?,
          continuation: self
            .intern_scalar_cont_ptr(*continuation, scalar_store)?,
        },
        Let { var, body, saved_env, continuation } => Continuation::Let {
        ...
        LetRec { var, body, saved_env, continuation } => Continuation::LetRec {
          var: self.intern_scalar_ptr(*var, scalar_store)?,
          body: self.intern_scalar_ptr(*body, scalar_store)?,
          saved_env: self.intern_scalar_ptr(*saved_env, scalar_store)?,
          continuation: self
  ...
  pub fn intern_scalar_ptr(
    ...
    match (tag, expr) {
      ...
      (Tag::Thunk, Some(Thunk(t))) => {
        ...
        let continuation =
          self.intern_scalar_cont_ptr(t.continuation, scalar_store)?;
        Some(self.intern_thunk(crate::store::Thunk { value, continuation }))
      },
  ...
  fn intern_sym_by_full_name<T: AsRef<str>>(&mut self, name: T) -> Ptr<F> {
    ...
    // We need to intern each of the path segments individually, so they will be
    // in the store. Otherwise, there can be an error when calling
    // `hash_symbol()` with an immutable store.
  ...
  pub fn get_lurk_sym<T: AsRef<str>>(
    &self,
    name: T,
    convert_case: bool,
  ) -> Option<Ptr<F>> {
  ...
  pub(crate) fn fetch_str(&self, ptr: &Ptr<F>) -> Option<&str> {
    ...
    let symbol =
      SymbolUsize::try_from_usize(ptr.1.idx()).expect("invalid pointer");
  ...
  pub(crate) fn fetch_fun(
    &self,
    ptr: &Ptr<F>,
  ) -> Option<&(Ptr<F>, Ptr<F>, Ptr<F>)> {
  ...
  pub(crate) fn fetch_uint(&self, ptr: &Ptr<F>) -> Option<UInt> {
    // If more UInt variants are added, the following assertion should be
    // relaxed to check for any of them.
  ...
  pub fn fetch(&self, ptr: &Ptr<F>) -> Option<Expression<F>> {
    ...
    match ptr.0 {
      ...
      Tag::Fun => {
        self.fetch_fun(ptr).map(|(a, b, c)| Expression::Fun(*a, *b, *c))
      },
      Tag::Thunk => {
        self.fetch_thunk(ptr).map(|thunk| Expression::Thunk(*thunk))
      },
  ...
  pub fn fetch_cont(&self, ptr: &ContPtr<F>) -> Option<Continuation<F>> {
    ...
    match ptr.0 {
      ...
      Call0 => self.call0_store.get_index(ptr.1.idx()).map(
        |(saved_env, continuation)| Continuation::Call0 {
          saved_env: *saved_env,
          continuation: *continuation,
        },
      ),
      Call => self.call_store.get_index(ptr.1.idx()).map(|(a, b, c)| {
        Continuation::Call { unevaled_arg: *a, saved_env: *b, continuation: *c }
      }),
      Call2 => self.call2_store.get_index(ptr.1.idx()).map(|(a, b, c)| {
        Continuation::Call2 { function: *a, saved_env: *b, continuation: *c }
      }),
      Tail => self
        .tail_store
        .get_index(ptr.1.idx())
        .map(|(a, b)| Continuation::Tail { saved_env: *a, continuation: *b }),
      Error => Some(Continuation::Error),
      Lookup => self
        .lookup_store
        .get_index(ptr.1.idx())
        .map(|(a, b)| Continuation::Lookup { saved_env: *a, continuation: *b }),
      Unop => self
        .unop_store
        .get_index(ptr.1.idx())
        .map(|(a, b)| Continuation::Unop { operator: *a, continuation: *b }),
      Binop => self.binop_store.get_index(ptr.1.idx()).map(|(a, b, c, d)| {
        Continuation::Binop {
          operator: *a,
          saved_env: *b,
          unevaled_args: *c,
      ...
      Binop2 => self.binop2_store.get_index(ptr.1.idx()).map(|(a, b, c)| {
        Continuation::Binop2 { operator: *a, evaled_arg: *b, continuation: *c }
      }),
      If => self
        .if_store
        .get_index(ptr.1.idx())
        .map(|(a, b)| Continuation::If { unevaled_args: *a, continuation: *b }),
      Let => self.let_store.get_index(ptr.1.idx()).map(|(a, b, c, d)| {
        Continuation::Let { var: *a, body: *b, saved_env: *c, continuation: *d }
      }),
      LetRec => self.letrec_store.get_index(ptr.1.idx()).map(|(a, b, c, d)| {
        Continuation::LetRec {
          var: *a,
          body: *b,
          saved_env: *c,
      ...
      Emit => self
        .emit_store
        .get_index(ptr.1.idx())
        .map(|continuation| Continuation::Emit { continuation: *continuation }),
    }
  }

  /// Mutable version of car_cdr to handle Str. `(cdr str)` may return a new str
  /// (the tail), which must be allocated.
  pub fn car_cdr_mut(
    &mut self,
    ptr: &Ptr<F>,
    ...
    match ptr.0 {
      ...
      Tag::Cons => match self.fetch(ptr) {
        ...
        Some(Expression::Opaque(_)) => {
          Err(Error("cannot destructure opaque Cons".into()))
        },
        _ => unreachable!(),
      },
      Tag::Str => {
  ...
  // This discovery is what led to get_expr_hash and the 'get' versions of
  // hash_cons, hash_sym, etc.
  ...
  pub fn get_hash_components_cont(&self, ptr: &ContPtr<F>) -> Option<[F; 8]> {
    ...
    let hash =
      match &cont {
        Outermost | Terminal | Dummy | Error => {
          self.get_hash_components_default()
        },
        Call0 { saved_env, continuation } => {
          self.get_hash_components_call0(saved_env, continuation)?
        },
        Call { unevaled_arg, saved_env, continuation } => self
          .get_hash_components_call(unevaled_arg, saved_env, continuation)?,
        ...
        Binop { operator, saved_env, unevaled_args, continuation } => self
          .get_hash_components_binop(
            operator,
            saved_env,
            unevaled_args,
            continuation,
        ...
        If { unevaled_args, continuation } => {
          self.get_hash_components_if(unevaled_args, continuation)?
        },
        Let { var, body, saved_env, continuation } => {
          self.get_hash_components_let(var, body, saved_env, continuation)?
        },
        LetRec { var, body, saved_env, continuation } => self
          .get_hash_components_let_rec(var, body, saved_env, continuation)?,
        Emit { continuation } => self.get_hash_components_emit(continuation)?,
      };

    Some([
      hash[0][0], hash[0][1], hash[1][0], hash[1][1], hash[2][0], hash[2][1],
      hash[3][0], hash[3][1],
    ])
  }
  ...
  fn get_hash_components_if(
    ...
    let def = [F::zero(), F::zero()];
    let unevaled_args =
      self.get_expr_hash(unevaled_args)?.into_hash_components();
    let cont = self.hash_cont(cont)?.into_hash_components();
  ...
  pub fn get_hash_components_thunk(&self, thunk: &Thunk<F>) -> Option<[F; 4]> {
    ...
    let continuation_hash =
      self.hash_cont(&thunk.continuation)?.into_hash_components();
  ...
  fn hash_fun(&self, fun: Ptr<F>, mode: HashScalar) -> Option<ScalarPtr<F>> {
    if fun.is_opaque() {
      Some(
        *self.opaque_map.get(&fun).expect("ScalarPtr for opaque Fun missing"),
      )
    }
    else {
  ...
  fn hash_cons(&self, cons: Ptr<F>, mode: HashScalar) -> Option<ScalarPtr<F>> {
    if cons.is_opaque() {
      return Some(
        *self.opaque_map.get(&cons).expect("ScalarPtr for opaque Cons missing"),
      );
    }
  ...
  fn hash_comm(&self, comm: Ptr<F>, mode: HashScalar) -> Option<ScalarPtr<F>> {
    if comm.is_opaque() {
      return Some(
        *self.opaque_map.get(&comm).expect("ScalarPtr for opaque Comm missing"),
      );
    }
  ...
  fn hash_symbol(&self, s: &Sym, mode: HashScalar) -> F {
    ...
    for name in path.iter() {
      ...
      let (prev_full_name, full_name) = if let Some(x) = full_name_acc.clone() {
        // We need to quote the path segment when constructing the incremental
        // full-name. This is because when symbols are interned by
        // full-name, the canonical name, which includes any necessary
        // quoting, is used.
        //
        // Eventually, we will remove symbol lookup by full-name, simplifying
        // this.
  ...
  fn all_hashes(
    &mut self,
    s: &str,
    initial_scalar_ptr: ScalarPtr<F>,
  ) -> Vec<F> {
    ...
    chars.fold(initial_scalar_ptr, |acc, char| {
      ...
      // This bypasses create_scalar_ptr but is okay because Chars are immediate
      // and don't need to be indexed.
      let c = ScalarPtr(Tag::Char.as_field(), c_scalar);
      let hash = self.hash_scalar_ptrs_2(&[c, acc]);
      // This bypasses create_scalar_ptr but is okay because we will call it to
      // correctly create each of these ScalarPtrs belwo, in
      // hash_string_mut_aux.
  ...
  fn hash_string_mut_aux(
    &mut self,
    mut s: im::Vector<char>,
    all_hashes: Vec<F>,
  ) -> F {
  ...
  pub fn ptr_eq(&self, a: &Ptr<F>, b: &Ptr<F>) -> Result<bool, Error> {
    // In order to compare Ptrs, we *must* resolve the hashes. Otherwise, we
    // risk failing to recognize equality of compound data with opaque data
    // in either element's transitive closure.
  ...
  /// Fill the cache for Scalars. Only Ptrs which have been interned since last
  /// hydration will be hashed, so it is safe to call this incrementally.
...
pub mod test {
  ...
  fn unit_scalar_ptr_ipld() {
    ...
    let cid =
      Cid::new_v1(Fr::to_multicodec(tag).unwrap(), Fr::to_multihash(dig));
  ...
  fn opaque_fun() {
    ...
    let limit = 10;
    {
      ...
      let (result, ..) =
        Evaluator::new(comparison_expr, empty_env, &mut store, limit)
          .eval()
          .unwrap();
    ...
    {
      // This test is important. It demonstrates that we can handle opaque data
      // in compound data being evaluated without this affecting equality
      // semantics.
  ...
  fn opaque_sym() {
    ...
    // When an opaque sym is inserted into a store which contains the same sym,
    // the store knows its identity. Should we just immediately coalesce and
    // never create an opaque version in that case? Probably not because
    // that may interact badly with explicit hiding to be implemented.
    // Let's defer any such considerations until we have a well-specified way of
    // segreting secret/private data.
    //
    // If implemented, the following commented test would pass.
    // assert_eq!(sym.fmt_to_string(&store), opaque_sym.fmt_to_string(&store));

    // For now, all opaque data remains opaque, even if the Store has enough
    ...
    // other_sym and other_opaque_sym are not equal, since the non-opaque symbol
    // was inserted after the opaque one. TODO: we could check for this and
    // fix when inserting non-opaque syms. If we decide to clarify opaque data
    // when possible, we should do this too.
    assert!(
      other_sym.fmt_to_string(&other_store)
        != other_opaque_sym.fmt_to_string(&other_store)
    );

    let num = num::Num::from_scalar(*sym_hash.value());
    assert_eq!(
      format!("<Opaque Sym {}>", Expression::Num(num).fmt_to_string(&store)),
      other_opaque_sym.fmt_to_string(&other_store)
    );

    // We need to insert a few opaque syms in other_store, in order to acquire a
    // raw_ptr that doesn't exist in store. Use that to check for a
    // malformed/missing opaque sym in store below.
    ...
    // other_opaque_sym doesn't exist at all in store, but it is recognized as
    // an opaque sym. It still prints 'normally', but attempts to fetch its
    // name detect this case. This shouldn't actually happen. The test just
    // exercise the code path which detects it.
  ...
  fn opaque_cons() {
    ...
    }
    {
      ...
      {
        let (result, ..) =
          Evaluator::new(comparison_expr2, empty_env, &mut store, limit)
            .eval()
            .unwrap();
        assert_eq!(nil, result.expr);
      }
  ...
  fn str_inner_fetch_aux(str: &str, hydrate: bool) {
    ...
    // Unless the cache is hydrated, the inner destructuring will not map the
    // ScalarPtr to corresponding Ptr.
    if hydrate {
  ...
  #[test]
  fn str_inner_fetch_hydrated() { str_inner_fetch_aux(r#" "ORANGE" "#, true); }
  ...
  #[test]
  fn empty_str_fetch_hydrated() { empty_str_fetch_aux(true); }

  #[test]
  fn empty_str_fetch_unhydrated() { empty_str_fetch_aux(false); }
