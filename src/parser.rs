...
impl<F: LurkField> Store<F> {
  ...
  fn read_number_aux<T: Iterator<Item = char>>(
    ...
    match self.read_number_suffix(chars) {
      Some(UInt::U64(_)) => {
        Ok(
          self.get_u64(acc.to_u64().ok_or_else(|| {
            Error::Syntax("Number too large for u64.".into())
          })?),
        )
      },
  ...
  /// If a number suffix can be read, return a UInt, specifying that suffix.
  /// Otherwise return None. Initially, only `u64` and `U64` are valid
  /// suffixes.
  ...
  pub(crate) fn intern_sym_in_package(
    &mut self,
    sym: Sym,
    package: &Package,
  ) -> Ptr<F> {
...
pub(crate) fn maybe_quote_symbol_name_string(
  symbol_name: &str,
...
fn skip_line_comment<T: Iterator<Item = char>>(
  chars: &mut PeekMoreIterator<T>,
) -> bool {
  while let Some(&c) = chars.peek() {
...
0987654321",
    ...
    // (format nil "~x" (mod (expt 2 256) (1+
    // #x73eda753299d7d483339d80809a1d80553bda402fffe5bfeffffffff00000000)))
  ...
  fn read_improper_list() {
    ...
    assert_eq!(s.read("(123 321)").unwrap(), s.read("(123 . ( 321 ))").unwrap())
  }
  #[test]
  fn read_print_expr() {
    let mut s = Store::<Fr>::default();
...
!asdf",
  ...
  fn read_string() {
    ...
    }
    {
      ...
      let res =
        s.fetch(&ptr).unwrap_or_else(|| panic!("failed to fetch: {:?}", input));
  ...
  fn read_with_comments() {
    ...
    let test =
      |store: &mut Store<Fr>, input: &str, expected: Option<Ptr<Fr>>| {
