...
impl<F: LurkField> Encodable for LightStore<F> {
  ...
  fn de(ld: &LightData) -> anyhow::Result<Self> {
    ...
    Ok(LightStore { scalar_map: pairs.into_iter().collect() })
  }
}
...
impl<F: LurkField> LightStore<F> {
  fn insert_scalar_string(
    ...
    for (ptr, c) in tail_ptrs.iter().rev().zip(s.chars().rev()) {
      ...
      store.insert_scalar_expression(
        *ptr,
        Some(ScalarExpression::Str(tail.clone())),
      );
    }
    Ok(s)
  }
  ...
  fn insert_scalar_symbol(
    ...
    if let Some(old_value) = store
      .insert_scalar_expression(ptr0, Some(ScalarExpression::Sym(path.clone())))
    {
    ...
    for (ptr, string) in tail_ptrs.iter().rev().zip(path.path().iter().rev()) {
      ...
      store.insert_scalar_expression(
        *ptr,
        Some(ScalarExpression::Sym(tail.clone())),
      );
    }
    Ok(path)
  }
...
impl<F: LurkField> Encodable for LightExpr<F> {
  fn ser(&self) -> LightData {
    match self {
      ...
      LightExpr::Comm(f, x) => LightData::Cell(vec![
        LightData::Atom(vec![3u8]),
        FWrap(*f).ser(),
        x.ser(),
      ]),
      LightExpr::Num(f) => {
        LightData::Cell(vec![LightData::Atom(vec![]), FWrap(*f).ser()])
      },
      LightExpr::Char(f) => {
        LightData::Cell(vec![LightData::Cell(vec![]), FWrap(*f).ser()])
      },
