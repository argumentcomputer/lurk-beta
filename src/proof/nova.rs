...
impl<'a, F: LurkField> Prover<'a, F> for NovaProver<F> {
  ...
  fn new(chunk_frame_count: usize) -> Self {
    NovaProver::<F> { chunk_frame_count, _p: PhantomData::<F> }
  }

  fn chunk_frame_count(&self) -> usize { self.chunk_frame_count }
}
...
impl<F: LurkField> NovaProver<F> {
  ...
  pub fn evaluate_and_prove<'a>(
    ...
    let circuits =
      MultiFrame::from_frames(self.chunk_frame_count(), &frames, store);
...
impl<'a> Proof<'a> {
  pub fn prove_recursively(
    ...
    for circuit_primary in circuits.iter() {
      ...
      if debug {
        // For debugging purposes, synthesize the circuit and check that the
  ...
  fn z0_secondary() -> Vec<S2> { vec![<G2 as Group>::Scalar::zero()] }
}
...
mod tests {
  ...
  fn nova_test_full_aux(
    ...
    let multiframes =
      MultiFrame::from_frames(nova_prover.chunk_frame_count(), &frames, s);
  ...
  #[test]
  #[should_panic]
  // This tests the testing mechanism. Since the supplied expected value is
  // wrong, the test should panic on an assertion failure.
  ...
  // The following functional commitment tests were discovered to fail. They are
  // commented out (as tests) for now so they can be addressed independently
  // in future work.
  ...
  fn outer_prove_complicated_functional_commitment() {
    ...
    let expr = "(let ((f (commit (let ((nums '(1 2 3))) (lambda (f) (f \
                nums)))))
                          (in (letrec ((sum-aux (lambda (acc nums)
                                              (if nums
                                                (sum-aux (+ acc (car nums)) \
                (cdr nums))
                                                acc)))
                                   (sum (sum-aux 0)))
                             (lambda (nums)
