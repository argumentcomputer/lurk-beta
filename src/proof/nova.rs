...
use bellperson::{
  gadgets::num::AllocatedNum, ConstraintSystem, SynthesisError,
};
...
impl<'a, F: LurkField> Prover<'a, F> for NovaProver<F> {
  ...
  fn new(reduction_count: usize) -> Self {
    NovaProver::<F> { reduction_count, _p: PhantomData::<F> }
  }
...
impl<F: LurkField> NovaProver<F> {
  ...
  pub fn evaluate_and_prove<'a>(
    ...
    let circuits =
      MultiFrame::from_frames(self.reduction_count(), &frames, store);
    ...
    CS: ConstraintSystem<F>,
  {
    ...
    let (new_expr, new_env, new_cont) = match self.frames.as_ref() {
      Some(frames) => {
        ...
        self.synthesize_frames(
          cs, s, input_expr, input_env, input_cont, frames, &g,
        )
      },
      None => {
...
impl<'a> Proof<'a> {
  pub fn prove_recursively(
    ...
    assert_eq!(circuits[0].frames.as_ref().unwrap().len(), num_iters_per_step);
    let (_circuit_primary, circuit_secondary) =
      C1::<'a>::circuits(num_iters_per_step);
  ...
  pub fn compress(self, pp: &'a PublicParams) -> Result<Self, ProofError> {
    match &self {
      Self::Recursive(recursive_snark) => Ok(Self::Compressed(Box::new(
        CompressedSNARK::<_, _, _, _, SS1, SS2>::prove(
          &pp.pp,
          &pp.pk,
          recursive_snark,
        )?,
  ...
  pub fn verify(
    ...
    Ok(
      zi_primary == zi_primary_verified
        && zi_secondary == zi_secondary_verified,
    )
  }
...
mod tests {
  ...
  fn nova_test_full_aux2(
    ...
    let frames = nova_prover.get_evaluation_frames(expr, e, s, limit).unwrap();

    let multiframes =
      MultiFrame::from_frames(nova_prover.reduction_count(), &frames, s);

    let len = multiframes.len();

    let adjusted_iterations =
      nova_prover.expected_total_iterations(expected_iterations);
    ...
    blank.synthesize(&mut cs_blank).expect("failed to synthesize blank");
  ...
  fn test_prove_if() {
    ...
    let expected = s.num(5);
    let terminal = s.get_cont_terminal();
    test_aux(s, "(if t 5 6)", Some(expected), None, Some(terminal), None, 3);

    let expected = s.num(6);
    let terminal = s.get_cont_terminal();
    test_aux(s, "(if nil 5 6)", Some(expected), None, Some(terminal), None, 3)
  }
  ...
  fn test_prove_if_end_is_nil_error() {
    ...
    let expected = s.num(5);
    let error = s.get_cont_error();
    test_aux(s, "(if nil 5 6 7)", Some(expected), None, Some(error), None, 2)
  }
  ...
  fn test_prove_emit_output() {
    ...
    let expected = s.num(123);
    let terminal = s.get_cont_terminal();
    test_aux(s, "(emit 123)", Some(expected), None, Some(terminal), None, 3);
  }
  ...
  fn test_prove_evaluate_sum() {
    ...
    let terminal = s.get_cont_terminal();
    test_aux(s, "(+ 2 (+ 3 4))", Some(expected), None, Some(terminal), None, 6);
  }
  ...
  fn test_prove_current_env_simple() {
    ...
    let expected = s.nil();
    let terminal = s.get_cont_terminal();
    test_aux(s, "(current-env)", Some(expected), None, Some(terminal), None, 1);
  }
  ...
  fn test_prove_current_env_rest_is_nil_error() {
    ...
    let error = s.get_cont_error();
    test_aux(s, "(current-env a)", Some(expected), None, Some(error), None, 1);
  }
  ...
  fn test_prove_zero_arg_lambda6() {
    ...
    let expected = s.num(123);
    let error = s.get_cont_error();
    test_aux(s, "((emit 123))", Some(expected), None, Some(error), None, 5);
  }
