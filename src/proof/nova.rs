...
impl<'a, F: LurkField, C: Coprocessor<F>> MultiFrame<'a, F, IO<F>, Witness<F>, C> {
    fn compute_witness(&self, s: &Store<F>) -> WitnessCS<F> {
        ...
        use crate::tag::Tag;
        let expr = s.get_expr_hash(&input.expr).unwrap();
        let env = s.get_expr_hash(&input.env).unwrap();
...
impl<'a: 'b, 'b, C: Coprocessor<S1>> Proof<'a, C> {
    ...
    pub fn prove_recursively(
        ...
        if CONFIG.parallelism.recursive_steps.is_parallel() {
            let cc = circuits
                .iter()
            ...
            crossbeam::thread::scope(|s| {
                ...
                for circuit_primary in cc.iter() {
                    ...
                    assert!(res.is_ok());
                    recursive_snark = Some(res.expect("result missing"));
                }
            })
            .unwrap();
        } else {
