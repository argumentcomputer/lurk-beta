...
use bellperson::{
  groth16::{
    self,
    aggregate::{
      aggregate_proofs_and_instances,
      setup_fake_srs,
      verify_aggregate_proof_and_aggregate_instances,
      AggregateProofAndInstance,
      AggregateVersion,
      GenericSRS,
      VerifierSRS,
    },
...
const DUMMY_RNG_SEED: [u8; 16] = [
  0x01, 0x03, 0x02, 0x04, 0x05, 0x07, 0x06, 0x08, 0x09, 0x0a, 0x0b, 0x0c, 0x0d,
  0x0c, 0x0b, 0x0a,
];

pub static INNER_PRODUCT_SRS: Lazy<GenericSRS<Bls12>> =
  Lazy::new(|| load_srs().unwrap());
...
impl Groth16Prover<Bls12> {
  pub fn create_groth_params(
    ...
    let multiframe = MultiFrame::blank(chunk_frame_count);

    // WARNING: These parameters are totally bogus. Real Groth16 parameters need
    // to be generated by a trusted setup. We create them
    // *deterministically* from a seeded RNG so that multiple runs will
    // create the same 'random' parameters. If you use these parameters in
    // production, anyone can make fake proofs.
    let rng = &mut XorShiftRng::from_seed(DUMMY_RNG_SEED);
    let params =
      groth16::generate_random_parameters::<Bls12, _, _>(multiframe, rng)?;
  ...
  pub fn outer_prove<R: RngCore + Clone>(
    ...
    if proofs.len().count_ones() != 1 || proofs.len() < 2 {
      ...
      let dummy_proof =
        self.prove(dummy_multiframe.clone(), params, &mut rng).unwrap();
  ...
  pub fn verify_groth16_proof(
    // multiframe need not have inner frames populated for verification
    // purposes.
...
pub struct PublicParams<E: Engine + MultiMillerLoop>(
  pub groth16::Parameters<E>,
);
...
impl Prover<'_, Scalar> for Groth16Prover<Bls12> {
  ...
  fn new(chunk_frame_count: usize) -> Self {
    Groth16Prover { chunk_frame_count, _p: PhantomData::<Bls12> }
  }
...
impl
  MultiFrame<
    '_,
    <Bls12 as Engine>::Fr,
    IO<<Bls12 as Engine>::Fr>,
    Witness<<Bls12 as Engine>::Fr>,
  >
{
  pub fn verify_groth16_proof(
    self,
...
fn verify_sequential_groth16_proofs(
  ...
  for (i, (multiframe, proof)) in multiframe_proofs.iter().enumerate() {
    ...
    if !multiframe.clone().verify_groth16_proof(&pvk, proof.clone())? {
      return Ok(false);
    }
  }
...
mod tests {
  ...
  fn outer_prove_aux0(
    ...
    if check_constraint_systems {
      ...
      let frames =
        Evaluator::generate_frames(expr, e, s, limit, padding_predicate)
          .unwrap();
      s.hydrate_scalar_cache();

      let multi_frames =
        MultiFrame::from_frames(DEFAULT_CHUNK_FRAME_COUNT, &frames, s);

      let cs = groth_prover.outer_synthesize(&multi_frames).unwrap();

      let _adjusted_iterations =
        groth_prover.expected_total_iterations(expected_iterations);

      if !debug {
        assert_eq!(
          expected_iterations,
          Frame::significant_frame_count(&frames)
        );
        // This test fails sometimes because we are using outer_synthesize to
        // get the frames. That method only really exists to let us test
        // synthesis without proofs, and it doesn't duplicate
        // all the padding logic required for SnarkPack. It might be nice to
        // eventually refactor such taht it does, in which case this
        // check will be useful. So let's leave it around for now.
      ...
      let constraint_systems_verified =
        verify_sequential_css::<Scalar>(&cs).unwrap();
  ...
  pub fn check_cs_deltas(
    ...
    let blank_frame =
      MultiFrame::<Scalar, _, _>::blank(DEFAULT_CHUNK_FRAME_COUNT);
    blank_frame.synthesize(&mut cs_blank).expect("failed to synthesize");
  ...
  fn outer_prove_chained_functional_commitment() {
    ...
    let (evaled, ..) =
      Evaluator::new(fun_src, empty_sym_env(&s), &mut s, limit).eval().unwrap();
    ...
    let (output, _iterations, _emitted) =
      Evaluator::new(input, empty_sym_env(&s), &mut s, limit).eval().unwrap();
