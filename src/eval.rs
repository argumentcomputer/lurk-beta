...
use crate::store;
use crate::store::{
  ContPtr, Continuation, Expression, NamedConstants, Pointer, Ptr, Store,
  Thunk, TypePredicates,
};
          ...
          Expression::Fun(arg, body, closed_env) => {
            if arg == c.dummy.ptr() {
              if body.is_nil() {
                ...
                let (body_form, end) = cons_witness.car_cdr_named(
                  ConsName::FunBody,
                  store,
                  &body,
                )?;
