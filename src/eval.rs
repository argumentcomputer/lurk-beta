...
struct ResultFrame<'a, F: LurkField>(
  Result<FrameIt<'a, Witness<F>, F>, ReductionError>,
);
    ...
    if cont.tag() == ContTag::Terminal {
      ...
      match expr.tag() {
                ...
                match var_or_rec_binding.tag() {
                  Tag::Sym => {
                    ...
                    if v == expr {
                      ...
                      // Pass the binding's value to the continuation in a
                      // thunk.
                      ...
                      match cont.tag() {
                        ContTag::Lookup => {
                          // If performing a lookup, continue with remaining
                          // env.
        ...
        Tag::Cons => {
          ...
          let (head, rest) =
            cons_witness.car_cdr_named(ConsName::Expr, store, &expr)?;
            ...
            || head == store.lurk_sym("letrec")
          {
            ...
            if !rest_body.is_nil() || body.is_nil() {
              ...
              let (binding1, rest_bindings) = cons_witness.car_cdr_named(
                ConsName::ExprCadr,
                store,
                &bindings,
              )?;
              ...
              if !end.is_nil() {
                ...
                let expanded = if rest_bindings.is_nil() {
                  ...
                  // We know body is a proper list equivalent to (body1), if
                  // this branch was taken, since end is nil.
            ...
            let (arg1, end) = cons_witness.car_cdr_mut_named(
              ConsName::ExprCdr,
              store,
              &rest,
            )?;
            if !end.is_nil() {
              ...
              Control::Return(
                ...
                cont_witness.intern_named_cont(
                  ContName::NewerCont,
                  store,
                  Continuation::Unop { operator: Op1::Car, continuation: cont },
                ),
              )
            }
          }
            ...
            if !end.is_nil() {
              ...
              Control::Return(
                ...
                cont_witness.intern_named_cont(
                  ContName::NewerCont,
                  store,
                  Continuation::Unop { operator: Op1::Cdr, continuation: cont },
                ),
              )
            }
          }
            ...
            if !end.is_nil() {
              ...
              Control::Return(
                ...
                cont_witness.intern_named_cont(
                  ContName::NewerCont,
                  store,
                  Continuation::Unop { operator: Op1::Num, continuation: cont },
                ),
              )
            }
          }
            ...
            if !end.is_nil() {
              ...
              Control::Return(
                ...
                cont_witness.intern_named_cont(
                  ContName::NewerCont,
                  store,
                  Continuation::Unop { operator: Op1::U64, continuation: cont },
                ),
              )
            }
          }
            ...
            Control::Return(
              ...
              cont_witness.intern_named_cont(
                ContName::NewerCont,
                store,
                Continuation::If { unevaled_args: more, continuation: cont },
              ),
            )
          }
          else if head == store.lurk_sym("current-env") {
            if !rest.is_nil() {
              // TODO: This returns current-env even in the error condition to
              // agree with the circuit, but it may be that the
              // circuit should be stricter and return expr as in
            ...
            if args.is_nil() {
              Control::Return(
                ...
                cont_witness.intern_named_cont(
                  ContName::NewerCont,
                  store,
                  Continuation::Call0 { saved_env: env, continuation: cont },
                ),
              )
            }
            else {
...
pub fn reduce_with_witness<F: LurkField>(
  ...
  let (control, closure_to_extend) = reduce_with_witness_inner(
    expr,
    env,
    cont,
    store,
  ...
  let control = match cont.tag() {
      ...
      .ok_or_else(|| store::Error("Fetch failed".into()))?
    {
      Continuation::Unop { operator, continuation } => {
        let val = match operator {
          Op1::Car => {
            match cons_witness.car_cdr_mut_named(
      ...
      .ok_or_else(|| store::Error("Fetch failed".into()))?
    {
      Continuation::Binop {
        ...
        let (arg2, rest) = cons_witness.car_cdr_named(
        ...
        if operator == Op2::Begin {
          if rest.is_nil() {
            ...
            let begin_again = cons_witness.cons_named(
              ConsName::Begin,
              store,
              begin,
              unevaled_args,
      ...
      Continuation::Binop2 { operator, evaled_arg, continuation } => {
        ...
        let result = match (
          ...
          _ => match operator {
            ...
            Op2::Cons => cons_witness.cons_named(
              ConsName::TheCons,
              store,
              evaled_arg,
              arg2,
      ...
      .ok_or_else(|| store::Error("Fetch failed".into()))?
    {
      Continuation::If { unevaled_args, continuation } => {
        ...
        let (arg1, more) = cons_witness.car_cdr_named(
          ConsName::UnevaledArgs,
          store,
          &unevaled_args,
        )?;
        ...
        let (arg2, end) = cons_witness.car_cdr_named(
...
fn make_tail_continuation<F: LurkField>(
  ...
  match continuation.tag() {
    ...
    _ => cont_witness.intern_named_cont(
      ContName::NewerCont2,
      store,
      Continuation::Tail { saved_env: env, continuation },
    ),
  }
...
where IO<F>: Copy
{
  ...
  pub fn eval(
    &mut self,
  ) -> Result<(IO<F>, usize, Vec<Ptr<F>>), ReductionError> {
  ...
  pub fn iter(
    &mut self,
  ) -> Result<Take<FrameIt<'_, Witness<F>, F>>, ReductionError> {
    let initial_input = self.initial();
...
fn extend_rec<F: LurkField>(
  ...
  match var_or_binding.tag() {
    ...
    Tag::Sym | Tag::Nil => {
      ...
      let res =
        cons_witness.cons_named(ConsName::ExtendedRec, store, list, env);
    ...
    Tag::Cons => {
      let cons = cons_witness.cons_named(ConsName::NewRecCadr, store, var, val);
      let cons2 =
        cons_witness.cons_named(ConsName::NewRec, store, cons, binding_or_env);
      let res =
        cons_witness.cons_named(ConsName::ExtendedRec, store, cons2, rest);
...
fn extend_closure<F: LurkField>(
  ...
  match fun.tag() {
    ...
    _ => unreachable!(
      "fun.tag() stopped being Tag::Fun after already having been checked in \
       caller."
    ),
  }
}
