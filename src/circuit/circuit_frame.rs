...
pub fn add_clause_single<'a, F: LurkField>(
...
> {
    ...
    let (the_expr, the_env, the_cont, make_thunk_num) = {
        ...
        let tag_is_u64 = alloc_equal(&mut cs.namespace(|| "tag_is_u64"), result.tag(), &g.u64_tag)?;
...
mod tests {
    ...
    fn num_self_evaluating() {
        ...
        let test_with_output = |output: IO<Fr>, expect_success: bool, store: &Store<Fr>| {
            ...
            //println!("{}", print_cs(&cs));
            assert_eq!(13591, cs.num_constraints());
            assert_eq!(13, cs.num_inputs());
            assert_eq!(13234, cs.aux().len());
