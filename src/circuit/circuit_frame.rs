...
use super::gadgets::constraints::{
  self, alloc_equal, alloc_equal_const, alloc_is_zero, and_v, div,
  enforce_implication, or, or_v, pick, pick_const, sub,
};
use crate::circuit::circuit_frame::constraints::{
  add, allocate_is_negative, boolean_to_num, enforce_pack, linear, mul,
};
use crate::circuit::gadgets::hashes::{
  AllocatedConsWitness, AllocatedContWitness,
};
...
impl<'a, F: LurkField, T: Clone + Copy, W: Copy> CircuitFrame<'a, F, T, W> {
  pub fn blank() -> Self {
    Self { store: None, input: None, output: None, witness: None }
  }
...
impl<'a, F: LurkField, T: Clone + Copy + std::cmp::PartialEq, W: Copy>
  MultiFrame<'a, F, T, W>
{
  pub fn blank(count: usize) -> Self {
    Self { store: None, input: None, output: None, frames: None, count }
  }
  ...
  pub fn from_frames(
    ...
    for chunk in frames.chunks(count) {
      ...
      let last_circuit_frame =
        *inner_frames.last().expect("chunk must not be empty");
  ...
  pub(crate) fn make_dummy(
    ...
    Self { store: Some(store), input, output, frames, count }
  }
...
impl<F: LurkField> Circuit<F> for MultiFrame<'_, F, IO<F>, Witness<F>> {
  fn synthesize<CS: ConstraintSystem<F>>(
    ...
    let mut synth = |store,
      ...
      // End public inputs.
      ////////////////////////////////////////////////////////////////////////////////
      let g = GlobalAllocations::new(
        &mut cs.namespace(|| "global_allocations"),
        store,
      )?;

      let (new_expr, new_env, new_cont) = self.synthesize_frames(
...
> {
  ...
  // --
  let reduce_sym_not_dummy =
    expr.is_sym(&mut cs.namespace(|| "reduce_sym_not_dummy"))?;
  ...
  let reduce_sym_not_dummy =
    and!(cs, &reduce_sym_not_dummy, &cont_is_not_terminal_or_error)?;
  ...
  // --
  let expr_is_cons =
    expr.is_cons(&mut cs.namespace(|| "reduce_cons_not_dummy0"))?;

  let reduce_cons_not_dummy =
    and!(cs, &expr_is_cons, &cont_is_not_terminal_or_error)?;
  ...
  let apply_continuation_make_thunk: AllocatedNum<F> =
    apply_continuation_results.3;
...
fn reduce_sym<F: LurkField, CS: ConstraintSystem<F>>(
  ...
  (AllocatedPtr<F>, AllocatedPtr<F>, AllocatedContPtr<F>, AllocatedNum<F>),
  SynthesisError,
> {
  let output_expr = AllocatedPtr::alloc_ptr(
  ...
  let output_env =
    AllocatedPtr::alloc_ptr(&mut cs.namespace(|| "output_env"), store, || {
      witness
        .as_ref()
        .map(|w| &w.prethunk_output_env)
        .ok_or(SynthesisError::AssignmentMissing)
    })?;
  let output_cont = AllocatedContPtr::alloc_cont_ptr(
  ...
  let sym_is_t =
    expr.alloc_equal(&mut cs.namespace(|| "sym is t"), &g.t_ptr)?;
  ...
  let binding_is_nil =
    binding.is_nil(&mut cs.namespace(|| "binding is nil"), g)?;
  let binding_not_nil = binding_is_nil.not();
  let binding_is_cons =
    binding.is_cons(&mut cs.namespace(|| "binding_is_cons"))?;
  ...
  let var_or_rec_binding_is_sym = var_or_rec_binding
    .is_sym(&mut cs.namespace(|| "var_or_rec_binding_is_sym"))?;
  let var_or_rec_binding_is_cons = var_or_rec_binding
    .is_cons(&mut cs.namespace(|| "var_or_rec_binding_is_cons"))?;
  ...
  let with_other_binding =
    and!(cs, &with_binding, &var_or_rec_binding_is_sym_or_cons.not())?;
  ...
  // NOTE: this allocation is unconstrained. See necessary constraint immediately below.
  let (fun_hash, fun_arg, fun_body, fun_closed_env) =
    Ptr::allocate_maybe_fun_unconstrained(
  ...
  let smaller_rec_env_is_nil = smaller_rec_env
    .is_nil(&mut cs.namespace(|| "smaller_rec_env_is_nil"), g)?;
  ...
  let smaller_rec_env_not_dummy =
    and!(cs, &smaller_rec_env_not_nil, &otherwise_and_v2_not_expr)?;
  ...
  let env_to_use =
    pick_ptr!(cs, &smaller_rec_env_is_nil, &smaller_env, &rec_extended_env)?;
  ...
  let with_sym_binding_unmatched =
    and!(cs, &with_sym_binding, &v_is_expr1.not())?;
  ...
  let with_cons_binding_unmatched =
    and!(cs, &with_cons_binding, &v2_is_expr.not())?;
  ...
  let output_cont_should_be_error =
    or!(cs, &with_other_binding, &needed_env_missing, &needed_binding_missing)?;
  ...
  implies!(cs, &output_expr_should_be_val_to_use, &output_expr_is_val_to_use);
  implies!(cs, &output_cont_should_be_error, &output_expr_is_expr);

  // env
  implies!(cs, &output_env_should_be_env, &output_env_is_env);
  implies!(cs, &output_env_should_be_smaller_env, &output_env_is_smaller_env);
  implies!(cs, &output_env_should_be_env_to_use, &output_env_is_env_to_use);
...
> {
  ...
  let head_is_let_or_letrec =
    and!(cs, &head_is_let_or_letrec0, &head_is_a_sym)?;
  ...
  let head_potentially_fun_type =
    or!(cs, &head_is_a_sym, &head_is_a_cons, &head_is_fun)?;
  let head_potentially_fun =
    and!(cs, &head_potentially_fun_type, &head_is_any.not())?;
  ...
  let expr_cadr_not_dummy0 =
    or!(cs, &arg1_is_cons, &arg1_is_nil, &arg1_is_str)?;
  let expr_cadr_not_dummy =
    and!(cs, &expr_cdr_not_dummy, &expr_cadr_not_dummy0, &head_is_lambda)?;
  ...
  let (lambda_expr, lambda_env, lambda_cont) = {
    ...
    let cdr_args_is_nil =
      cdr_args.is_nil(&mut cs.namespace(|| "cdr_args_is_nil"), g)?;
    ...
    let lambda_not_dummy =
      and!(cs, &head_is_lambda, not_dummy, &more_is_nil.not())?;
  ...
  let bindings_is_nil =
    bindings.is_nil(&mut cs.namespace(|| "bindings_is_nil"), g)?;
  let bindings_is_cons =
    bindings.is_cons(&mut cs.namespace(|| "bindings_is_cons"))?;
  let body_is_nil = body.is_nil(&mut cs.namespace(|| "body_is_nil"), g)?;
  let rest_body_is_nil =
    rest_body.is_nil(&mut cs.namespace(|| "rest_body_is_nil"), g)?;
  ...
  let (
    ...
    let var_let_letrec_is_nil = var_let_letrec
      .is_nil(&mut cs.namespace(|| "var_let_letrec_is_nil"), g)?;
    let var_let_letrec_is_list =
      or!(cs, &var_let_letrec_is_sym, &var_let_letrec_is_nil)?;

    let expr_caaadr_not_dummy =
      and!(cs, &expr_caadr_not_dummy, &var_let_letrec_is_list)?;
  ...
  // head == / preimage
  /////////////////////////////////////////////////////////////////////////////
  let quotient_continuation_components: &[&dyn AsAllocatedHashComponents<F>;
     4] = &[&[&g.op2_quotient_tag, &g.default_num], env, &more, cont];
  hash_default_results.add_hash_input_clauses(
    c.quotient.value(),
    &g.binop_cont_tag,
  ...
  let numequal_continuation_components: &[&dyn AsAllocatedHashComponents<F>;
     4] = &[&[&g.op2_numequal_tag, &g.default_num], env, &more, cont];
  hash_default_results.add_hash_input_clauses(
    c.num_equal.value(),
    &g.binop_cont_tag,
  ...
  // head == <= preimage
  /////////////////////////////////////////////////////////////////////////////
  let less_equal_continuation_components: &[&dyn AsAllocatedHashComponents<F>;
     4] = &[&[&g.op2_less_equal_tag, &g.default_num], env, &more, cont];
  hash_default_results.add_hash_input_clauses(
    c.less_equal.value(),
    &g.binop_cont_tag,
  ...
  // head == >= preimage
  /////////////////////////////////////////////////////////////////////////////
  let greater_equal_continuation_components: &[&dyn AsAllocatedHashComponents<F>;
     4] = &[&[&g.op2_greater_equal_tag, &g.default_num], env, &more, cont];
  hash_default_results.add_hash_input_clauses(
  ...
  let newer_cont_not_dummy0 =
    or!(cs, &newer_cont_binop, &newer_cont_unop, &newer_cont_let_letrec)?;
  ...
  let newer_cont_not_dummy =
    and!(cs, &newer_cont_not_dummy0, not_dummy, &is_dotted_error.not())?;

  let newer_cont = AllocatedContPtr::construct_named(
    &mut cs
      .namespace(|| "reduce cons newer_cont construction from hash components"),
    ContName::NewerCont,
    &components_results[0], // continuation tag
    &[
      &[&components_results[1], &components_results[2]]
        as &dyn AsAllocatedHashComponents<F>,
      &[&components_results[3], &components_results[4]]
        as &dyn AsAllocatedHashComponents<F>,
      &[&components_results[5], &components_results[6]]
        as &dyn AsAllocatedHashComponents<F>,
      &[&components_results[7], &components_results[8]]
        as &dyn AsAllocatedHashComponents<F>,
    ],
    allocated_cont_witness,
  ...
  let (res, continuation) = {
    ...
    let more_args_is_nil =
      more.is_nil(&mut cs.namespace(|| "more_args_is_nil"), g)?;
  ...
  Ok((result_expr, result_env, result_cont, result_apply_cont.clone()))
}
...
> {
  ...
  let apply_continuation_components_not_dummy =
    and!(cs, &cont_is_trivial.not(), not_dummy)?;
  ...
  let call0_components: &[&dyn AsAllocatedHashComponents<F>; 4] =
    &[&saved_env, &continuation, default_num_pair, default_num_pair];
  hash_default_results.add_hash_input_clauses(
  ...
  let (commitment, commitment_secret, committed_expr) = {
    ...
    let is_op2_hide = operator.alloc_tag_equal(
      &mut cs.namespace(|| "is_op2_hide"),
      Op2::Hide.to_field(),
    )?;
    let is_op1_open = operator.alloc_tag_equal(
      &mut cs.namespace(|| "is_op1_open"),
      Op1::Open.to_field(),
    )?;
    ...
    let open_expr =
      AllocatedPtr::alloc(&mut cs.namespace(|| "open_expr"), || {
        Ok(store.hash_expr(&open_expr_ptr).unwrap())
      })?;

    let open_secret =
      AllocatedNum::alloc(&mut cs.namespace(|| "open_secret"), || {
    ...
    let secret =
      pick!(cs, &is_op1_open_or_secret, &open_secret, &commit_secret)?;
  ...
  let (unop_val, unop_continuation) = {
    ...
    let unop_op_is_car = op1.alloc_tag_equal(
      &mut cs.namespace(|| "unop_op_is_car"),
      Op1::Car.to_field(),
    )?;
    let unop_op_is_cdr = op1.alloc_tag_equal(
      &mut cs.namespace(|| "unop_op_is_cdr"),
      Op1::Cdr.to_field(),
    )?;
    let unop_op_is_car_or_cdr = or!(cs, &unop_op_is_car, &unop_op_is_cdr)?;

    let result_is_cons =
      result.is_cons(&mut cs.namespace(|| "result_is_cons"))?;
    ...
    let res_car =
      pick_ptr!(cs, &result_is_empty_str, &g.nil_ptr, &allocated_car)?;
    let res_cdr =
      pick_ptr!(cs, &result_is_empty_str, &g.empty_str_ptr, &allocated_cdr)?;
    ...
    let (u32_elem, u64_elem) = to_unsigned_integers(
      &mut cs.namespace(|| "Unop u32 and u64"),
      g,
      result.hash(),
    )?;
    ...
    let res = multi_case(
      ...
      &[
        &[
          CaseClause { key: Op1::Car.to_field(), value: res_car.tag() },
          CaseClause { key: Op1::Cdr.to_field(), value: res_cdr.tag() },
          CaseClause { key: Op1::Atom.to_field(), value: is_atom_ptr.tag() },
          CaseClause { key: Op1::Emit.to_field(), value: result.tag() },
          CaseClause { key: Op1::Commit.to_field(), value: commitment.tag() },
          CaseClause { key: Op1::Open.to_field(), value: committed_expr.tag() },
          CaseClause { key: Op1::Secret.to_field(), value: &g.num_tag },
          CaseClause { key: Op1::Num.to_field(), value: num.tag() },
          CaseClause { key: Op1::U64.to_field(), value: &g.u64_tag },
          CaseClause { key: Op1::Comm.to_field(), value: comm.tag() },
          CaseClause { key: Op1::Char.to_field(), value: &g.char_tag },
          CaseClause { key: Op1::Eval.to_field(), value: result.tag() },
        ],
        &[
          CaseClause { key: Op1::Car.to_field(), value: allocated_car.hash() },
          CaseClause { key: Op1::Cdr.to_field(), value: allocated_cdr.hash() },
          CaseClause { key: Op1::Atom.to_field(), value: is_atom_ptr.hash() },
          CaseClause { key: Op1::Emit.to_field(), value: result.hash() },
          CaseClause { key: Op1::Commit.to_field(), value: commitment.hash() },
          CaseClause {
            key: Op1::Open.to_field(),
            value: committed_expr.hash(),
          },
          CaseClause { key: Op1::Secret.to_field(), value: &commitment_secret },
          CaseClause { key: Op1::Num.to_field(), value: num.hash() },
          CaseClause { key: Op1::U64.to_field(), value: &u64_elem },
          CaseClause { key: Op1::Comm.to_field(), value: comm.hash() },
          CaseClause { key: Op1::Char.to_field(), value: &u32_elem },
          CaseClause { key: Op1::Eval.to_field(), value: result.hash() },
        ],
      ],
      &[&g.default_num, &g.default_num],
      g,
  ...
  let binop_components: &[&dyn AsAllocatedHashComponents<F>; 4] =
    &[&[op2, &g.default_num], result, &continuation, default_num_pair];
  hash_default_results.add_hash_input_clauses(
    ContTag::Binop.to_field(),
    &g.binop2_cont_tag,
    binop_components,
  ...
  let (newer_cont2, newer_cont2_not_dummy_num) =
    AllocatedContPtr::construct_named_and_not_dummy(
  ...
  let (the_expr, the_env, the_cont, newer_cont2_not_dummy) = {
    ...
    // NOTE: this allocation is unconstrained. See necessary constraint immediately below.
    let (fun_hash, arg_t, body_t, closed_env) =
      Ptr::allocate_maybe_fun_unconstrained(
    ...
    let args_is_dummy = arg_t
      .alloc_equal(&mut cs.namespace(|| "args_is_dummy"), &g.dummy_arg_ptr)?;

    let body_t_is_cons =
      body_t.is_cons(&mut cs.namespace(|| "body_t_is_cons"))?;
    let body_t_is_nil =
      body_t.is_nil(&mut cs.namespace(|| "body_t_is_nil"), g)?;

    let body_is_list = or!(cs, &body_t_is_cons, &body_t_is_nil)?;
    let zero_arg_call_not_dummy =
      and!(cs, &call0_not_dummy, &body_is_list, &args_is_dummy)?;
    ...
    let body_form_is_nil =
      body_form.is_nil(&mut cs.namespace(|| "body_form_is_nil"), g)?;
    ...
    let result_is_valid_zero_arg_fun =
      and!(cs, &result_is_valid_fun, &args_is_dummy)?;
    ...
    let newer_cont2_not_dummy0 =
      and!(cs, &continuation_is_tail.not(), &args_is_dummy, &result_is_fun)?;
  ...
  let (the_expr, the_env, the_cont, newer_cont2_not_dummy) = {
...

    {
      ...
      let cont_is_call2_and_not_dummy =
        and!(cs, &cont_is_call2_precomp, not_dummy)?;

      // NOTE: this allocation is unconstrained. See necessary constraint immediately below.
      let (hash, arg_t, body_t, closed_env) =
        Ptr::allocate_maybe_fun_unconstrained(
          &mut cs.namespace(|| "allocate fun"),
          store,
      ...
      let newer_cont2_not_dummy0 =
        and!(cs, &continuation_is_tail.not(), &cond)?;
      let newer_cont2_not_dummy = boolean_num!(cs, &newer_cont2_not_dummy0)?;
  ...
  let (the_expr, the_env, the_cont, newer_cont2_not_dummy) = {
    ...
    let op_is_begin = operator.alloc_tag_equal(
      &mut cs.namespace(|| "op_is_begin"),
      Op2::Begin.to_field(),
    )?;

    let rest_is_nil =
      allocated_rest.is_nil(&mut cs.namespace(|| "rest_is_nil"), g)?;
    let rest_not_nil = rest_is_nil.not();

    let begin = store.get_begin();

    let allocated_begin =
      AllocatedPtr::alloc_ptr(&mut cs.namespace(|| "begin"), store, || {
        Ok(&begin)
      })?;

    let begin_not_dummy =
      and!(cs, &op_is_begin, &binop_not_dummy, &rest_not_nil)?;
  ...
  let (the_expr, the_env, the_cont, make_thunk_num) = {
    ...
    let args_equal = arg1_final
      .alloc_equal(&mut cs.namespace(|| "args_equal"), &arg2_final)?;
    ...
    let op2_is_div = op2.alloc_tag_equal(
      &mut cs.namespace(|| "op2_is_div"),
      Op2::Quotient.to_field(),
    )?;
    let op2_is_mod = op2.alloc_tag_equal(
      &mut cs.namespace(|| "op2_is_mod"),
      Op2::Modulo.to_field(),
    )?;

    let op2_is_div_or_mod =
      or(&mut cs.namespace(|| "op2 is div or mod"), &op2_is_div, &op2_is_mod)?;
    ...
    let is_cons = op2.alloc_tag_equal(
    ...
    let is_cons_or_strcons =
      or(&mut cs.namespace(|| "is cons or strcons"), &is_cons, &is_strcons)?;
    ...
    let cons_not_dummy =
      and!(cs, &is_cons_or_strcons, &not_dummy, &invalid_strcons_tag.not())?;
    ...
    let val = case(
      ...
      op2.tag(),
      &[
        CaseClause { key: Op2::Sum.to_field(), value: &sum },
        CaseClause { key: Op2::Diff.to_field(), value: &diff },
        CaseClause { key: Op2::Product.to_field(), value: &product },
        CaseClause { key: Op2::Quotient.to_field(), value: &quotient },
        CaseClause { key: Op2::Equal.to_field(), value: args_equal_ptr.hash() },
        CaseClause {
          key: Op2::NumEqual.to_field(),
          value: args_equal_ptr.hash(),
        },
        CaseClause { key: Op2::Cons.to_field(), value: cons.hash() },
        CaseClause { key: Op2::StrCons.to_field(), value: cons.hash() },
        CaseClause { key: Op2::Hide.to_field(), value: commitment.hash() },
      ],
      &g.default_num,
      g,
    )?;

    let is_equal = op2.alloc_tag_equal(
    ...
    let op2_is_hide = op2.alloc_tag_equal(
    ...
    let op2_is_diff = op2.alloc_tag_equal(
      &mut cs.namespace(|| "op2_is_diff"),
      Op2::Diff.to_field(),
    )?;
    ...
    let coerce_to_u64_ptr =
      AllocatedPtr::from_parts(&g.u64_tag, &coerce_to_u64);
    ...
    let real_div_or_mor_and_b_is_zero =
      and!(cs, &not_dummy, &op2_is_div_or_mod, b_is_zero)?;
    ...
    let op2_not_num_or_u64_and_not_cons_or_strcons_or_hide_or_equal_or_num_equal =
      Boolean::and(
        &mut cs.namespace(|| {
          "not num and not cons or strcons or hide or equal or num_equal"
        }),
    ...
    let op2_is_hide_and_arg1_is_not_num =
      and!(cs, &op2_is_hide, &arg1_is_num.not())?;
    ...
    let op2_is_eval = op2.alloc_tag_equal(
      &mut cs.namespace(|| "op2_is_eval"),
      Op2::Eval.to_field(),
    )?;
  ...
  let (res, the_cont) = {
    ...
    let cont_is_if = cont.alloc_tag_equal(
    ...
    let condition_is_nil =
      condition.is_nil(&mut cs.namespace(|| "condition is nil"), g)?;
  ...
  let (body, extended_env, tail_cont, newer_cont2_not_dummy) = {
    ...
    let cont_is_let = cont.alloc_tag_equal(
    ...
    let extended_env_not_dummy =
      and!(cs, &let_cont_is_let, not_dummy, &cont_is_let)?;
  ...
  let (body, extended_env, return_cont, newer_cont2_not_dummy) = {
    ...
    let extend_rec_not_dummy =
      and!(cs, &letrec_cont_is_letrec_cont, not_dummy)?;
    ...
    let is_error = extended_env
      .alloc_equal(&mut cs.namespace(|| "is_error"), &g.error_ptr)?;
  ...
  let (the_expr, the_env, the_cont, make_thunk_num, newer_cont2_not_dummy) = {
    ...
    let other_unop_continuation =
      AllocatedContPtr::by_index(1, &continuation_components);

    let op1_is_emit = unop_op1.alloc_tag_equal(
      &mut cs.namespace(|| "op1_is_emit"),
      Op1::Emit.to_field(),
    )?;
    let op1_is_eval = unop_op1.alloc_tag_equal(
    ...
    let result_is_nil =
      result.is_nil(&mut cs.namespace(|| "result_is_nil"), g)?;

    let car_cdr_is_valid =
      or!(cs, &result_is_cons, &result_is_str, &result_is_nil)?;

    let op1_is_car = unop_op1.alloc_tag_equal(
      &mut cs.namespace(|| "op1_is_car"),
      Op1::Car.to_field(),
    )?;
    let op1_is_cdr = unop_op1.alloc_tag_equal(
      &mut cs.namespace(|| "op1_is_cdr"),
      Op1::Cdr.to_field(),
    )?;
    let op1_is_car_or_cdr = or!(cs, &op1_is_car, &op1_is_cdr)?;
    let car_cdr_is_invalid =
      and!(cs, &op1_is_car_or_cdr, &car_cdr_is_valid.not())?;

    let op1_is_comm = unop_op1.alloc_tag_equal(
      &mut cs.namespace(|| "op1_is_comm"),
      Op1::Comm.to_field(),
    )?;
    let op1_is_num = unop_op1.alloc_tag_equal(
      &mut cs.namespace(|| "op1_is_num"),
      Op1::Num.to_field(),
    )?;
    let op1_is_char = unop_op1.alloc_tag_equal(
      &mut cs.namespace(|| "op1_is_char"),
      Op1::Char.to_field(),
    )?;
    let op1_is_open = unop_op1.alloc_tag_equal(
      &mut cs.namespace(|| "op1_is_open"),
      Op1::Open.to_field(),
    )?;
    ...
    let op1_is_u64 = unop_op1.alloc_tag_equal(
      &mut cs.namespace(|| "op1_is_u64"),
      Op1::U64.to_field(),
    )?;
    ...
    let tag_is_num_or_comm_or_char =
      or!(cs, &tag_is_num_or_comm, &tag_is_char)?;
    let tag_is_num_or_comm_or_char_or_u64 =
      or!(cs, &tag_is_num_or_comm_or_char, &tag_is_u64)?;

    let comm_invalid_tag_error =
      and!(cs, &tag_is_num_or_comm.not(), &op1_is_comm)?;
    let num_invalid_tag_error =
      and!(cs, &tag_is_num_or_comm_or_char_or_u64.not(), &op1_is_num)?;
    let char_invalid_tag_error =
      and!(cs, &tag_is_num_or_char.not(), &op1_is_char)?;
    let open_invalid_tag_error =
      and!(cs, &tag_is_num_or_comm.not(), &op1_is_open)?;
    let secret_invalid_tag_error =
      and!(cs, &tag_is_num_or_comm.not(), &op1_is_secret)?;
    ...
    (the_expr, the_env, the_cont, make_thunk_num, newer_cont2_not_dummy)
  };
  ...
  let newer_cont2_not_dummy_ =
    equal_const!(cs, &newer_cont2_not_dummy_result_num, F::one())?;
...
pub fn comparison_helper<F: LurkField, CS: ConstraintSystem<F>>(
  ...
  let a_is_negative =
    allocate_is_negative(&mut cs.namespace(|| "enforce a is negative"), a)?;
  let b_is_negative =
    allocate_is_negative(&mut cs.namespace(|| "enforce b is negative"), b)?;
  let diff_is_negative = allocate_is_negative(
    &mut cs.namespace(|| "enforce diff is negative"),
    diff,
  )?;
...
pub fn to_unsigned_integer_helper<F: LurkField, CS: ConstraintSystem<F>>(
  ...
  let power_of_two_bn =
    BigUint::pow(&BigUint::from_u32(2).unwrap(), size.num_bits());
  ...
  enforce_pack(&mut cs.namespace(|| "enforce unsigned pack"), r_bits, &r_num)?;
...
pub fn to_unsigned_integers<F: LurkField, CS: ConstraintSystem<F>>(
  ...
  let field_elem_bits = maybe_unsigned
    .to_bits_le(&mut cs.namespace(|| "field element bit decomp"))?;
...
pub fn to_u64<F: LurkField, CS: ConstraintSystem<F>>(
  ...
  let field_bn = BigUint::from_bytes_le(field_elem.to_repr().as_ref());
  let field_elem_bits =
    maybe_u64.to_bits_le(&mut cs.namespace(|| "field element bit decomp"))?;
...
pub fn enforce_u64_div_mod<F: LurkField, CS: ConstraintSystem<F>>(
  ...
  let alloc_r_num =
    AllocatedNum::alloc(&mut cs.namespace(|| "r num"), || Ok(F::from_u64(r)))?;
  let alloc_q_num =
    AllocatedNum::alloc(&mut cs.namespace(|| "q num"), || Ok(F::from_u64(q)))?;
  let alloc_arg1_num =
    AllocatedNum::alloc(&mut cs.namespace(|| "arg1 num"), || {
      Ok(F::from_u64(arg1_u64))
    })?;
  let alloc_arg2_num =
    AllocatedNum::alloc(&mut cs.namespace(|| "arg2 num"), || {
  ...
  let b_is_zero =
    alloc_is_zero(&mut cs.namespace(|| "b is zero"), arg2.hash())?;
...
pub fn enforce_less_than_bound<F: LurkField, CS: ConstraintSystem<F>>(
  ...
  let diff_bound_num =
    sub(&mut cs.namespace(|| "bound minus num"), &bound, &num)?;
...
pub fn allocate_unconstrained_bignum<F: LurkField, CS: ConstraintSystem<F>>(
  ...
  let num =
    AllocatedNum::alloc(&mut cs.namespace(|| "num"), || Ok(ff)).unwrap();
  Ok(num)
}
...
fn car_cdr_named<F: LurkField, CS: ConstraintSystem<F>>(
  ...
  let maybe_cons_is_nil =
    maybe_cons.is_nil(&mut cs.namespace(|| "maybe_cons_is_nil"), g)?;
  ...
  if cons_not_dummy.get_value().unwrap_or(false)
    && !real_cons.get_value().unwrap_or(true)
  {
...
fn extend_rec<F: LurkField, CS: ConstraintSystem<F>>(
  ...
  let is_sym =
    var_or_binding.is_sym(&mut cs.namespace(|| "var_or_binding is sym"))?;
  let is_nil =
    var_or_binding.is_nil(&mut cs.namespace(|| "var_or_binding is nil"), g)?;
...
mod tests {
  ...
  fn num_self_evaluating() {
    ...
    let input = IO { expr: num, env, cont: store.intern_cont_outermost() };

    let (_, witness) = input.reduce(&mut store).unwrap();

    let public_params =
      Groth16Prover::create_groth_params(DEFAULT_REDUCTION_COUNT).unwrap();
    ...
    store.hydrate_scalar_cache();
    let test_with_output = |output: IO<Fr>,
                            expect_success: bool,
                            store: &Store<Fr>| {
      let mut cs = TestConstraintSystem::new();

      let mut cs_blank = MetricCS::<Fr>::new();
      let blank_multiframe = MultiFrame::<<Bls12 as Engine>::Fr, _, _>::blank(
        DEFAULT_REDUCTION_COUNT,
      );

      blank_multiframe.synthesize(&mut cs_blank).expect("failed to synthesize");

      let multiframes = MultiFrame::from_frames(
        DEFAULT_REDUCTION_COUNT,
        &[Frame { input, output, i: 0, witness }],
        store,
      );

      let multiframe = &multiframes[0];

      multiframe.clone().synthesize(&mut cs).expect("failed to synthesize");
      ...
      let proof =
        groth_prover.prove(multiframe.clone(), groth_params, &mut rng).unwrap();
      let cs_verified = cs.is_satisfied() && cs.verify(&public_inputs);
      let verified =
        multiframe.clone().verify_groth16_proof(&pvk, proof).unwrap();
    ...
    // Success
    {
      let output = IO { expr: num, env, cont: store.intern_cont_terminal() };
...

    {
      ...
      let bad_output_value =
        IO { expr: store.num(999), env, cont: store.intern_cont_terminal() };
  ...
  fn nil_self_evaluating() {
    ...
    let input = IO { expr: nil, env, cont: store.intern_cont_outermost() };

    let (_, witness) = input.reduce(&mut store).unwrap();
    store.hydrate_scalar_cache();

    let test_with_output =
      |output: IO<Fr>, expect_success: bool, store: &Store<Fr>| {
        let mut cs = TestConstraintSystem::<Fr>::new();

        let frame = Frame { input, output, i: 0, witness };
    ...
    // Success
    {
      let output = IO { expr: nil, env, cont: store.intern_cont_terminal() };
  ...
  fn t_self_evaluating() {
    ...
    let input = IO { expr: t, env, cont: store.intern_cont_outermost() };
    ...
    // Success
    {
      let output = IO { expr: t, env, cont: store.intern_cont_terminal() };
    ...
    // Failure
    {
      ...
      }
      {
        ...
        let bad_output_value =
          IO { expr: store.sym("S"), env, cont: store.intern_cont_terminal() };
        test_with_output(bad_output_value, false, &store);
      }
    }
  }
  ...
  fn fun_self_evaluating() {
    ...
    let input = IO { expr: fun, env, cont: store.intern_cont_outermost() };
    ...
    // Success
    {
      let output = IO { expr: fun, env, cont: store.intern_cont_terminal() };
  ...
  fn non_self_evaluating() {
    ...
    let input = IO { expr, env, cont: store.intern_cont_outermost() };
    ...
    {
      {
        ...
        let output = IO { expr, env, cont: store.intern_cont_terminal() };
  ...
  fn test_enforce_comparison() {
    ...
    let a = s.num(42);
    let alloc_a =
      AllocatedPtr::alloc_ptr(&mut cs.namespace(|| "a"), s, || Ok(&a)).unwrap();
    let b = s.num(43);
    let alloc_b =
      AllocatedPtr::alloc_ptr(&mut cs.namespace(|| "b"), s, || Ok(&b)).unwrap();
    let diff =
      sub(&mut cs.namespace(|| "sub"), alloc_a.hash(), alloc_b.hash()).unwrap();
  ...
  fn test_u64_op() {
    ...
    let a_u64 =
      to_u64(&mut cs.namespace(|| "u64 op"), &g, alloc_a.hash()).unwrap();
    assert!(cs.is_satisfied());
  ...
  fn test_u64_op_coercion() {
    ...
    let a_u64 =
      to_u64(&mut cs.namespace(|| "u64 op"), &g, alloc_pow2_64.hash()).unwrap();
    assert!(cs.is_satisfied());
  ...
  fn test_enforce_less_than_bound_corner_case() {
    ...
    let alloc_most_positive =
      ...
      .unwrap();
    let alloc_num =
      AllocatedNum::alloc(&mut cs.namespace(|| "num"), || Ok(Fr::from_u64(42)))
        .unwrap();
  ...
  fn test_enforce_less_than_bound() {
    ...
    let alloc_num =
      AllocatedNum::alloc(&mut cs.namespace(|| "num"), || Ok(Fr::from_u64(42)))
        .unwrap();
    let alloc_bound =
      AllocatedNum::alloc(&mut cs.namespace(|| "bound"), || {
        Ok(Fr::from_u64(43))
      })
      .unwrap();
    let cond = Boolean::Constant(true);
  ...
  fn test_enforce_less_than_bound_negative() {
    let mut cs = TestConstraintSystem::<Fr>::new();
    let alloc_num =
      AllocatedNum::alloc(&mut cs.namespace(|| "num"), || Ok(Fr::from_u64(43)))
        .unwrap();
    let alloc_bound =
      AllocatedNum::alloc(&mut cs.namespace(|| "bound"), || {
        Ok(Fr::from_u64(42))
  ...
  fn test_enforce_popcount() {
    ...
    for x in 0..128 {
      let a = s.num(x);
      let alloc_a =
        AllocatedPtr::alloc_ptr(&mut cs.namespace(|| x.to_string()), s, || {
          Ok(&a)
        })
        .unwrap();
      let bits = alloc_a
  ...
  fn test_to_u32() {
    ...
    let a_plus_power2_32_num =
      AllocatedNum::alloc(&mut cs.namespace(|| "pow(2, 32) + 2"), || Ok(v))
        .unwrap();

    let bits =
      a_plus_power2_32_num.to_bits_le(&mut cs.namespace(|| "bits")).unwrap();
  ...
  fn test_to_u64() {
    ...
    let a_plus_power2_64_num =
      AllocatedNum::alloc(&mut cs.namespace(|| "pow(2, 64) + 2"), || Ok(v))
        .unwrap();

    let bits =
      a_plus_power2_64_num.to_bits_le(&mut cs.namespace(|| "bits")).unwrap();
