...
use crate::{
  circuit::{
    circuit_frame::constraints::{
      add,
      allocate_is_negative,
      boolean_to_num,
      enforce_pack,
      linear,
      mul,
    ...
    gadgets::{
      ...
      hashes::{
        AllocatedConsWitness,
        AllocatedContWitness,
      },
      pointer::{
        AllocatedContPtr,
        AllocatedPtr,
        AsAllocatedHashComponents,
      },
    },
    ToInputs,
  },
  ...
  MultiFrame<'a, F, T, W>
{
  ...
  pub fn get_store(&self) -> &Store<F> { self.store.expect("store missing") }
  ...
  pub fn from_frames(
    ...
    for chunk in frames.chunks(count) {
      ...
      // Fill out the MultiFrame, if needed, and capture output of the final
      // actual frame.
...
impl<F: LurkField, W: Copy> Provable<F> for MultiFrame<'_, F, IO<F>, W> {
  ...
  fn chunk_frame_count(&self) -> usize { self.count }
}
      ...
      let (_, (new_expr, new_env, new_cont)) =
        frames.iter().fold((0, acc), |(i, allocated_io), frame| {
          if let Some(next_input) = frame.input {
            // Ensure all intermediate allocated I/O values match the provided
            // executation trace.
            assert_eq!(
...
> {
  ...
  // NOTE: this allocation is unconstrained. See necessary constraint
  // immediately below.
  ...
  let reduce_sym_not_dummy =
    and!(cs, &reduce_sym_not_dummy, &cont_is_not_terminal)?;
  ...
  let reduce_cons_not_dummy =
    and!(cs, &expr_is_cons, &cont_is_not_error, &cont_is_not_terminal)?;
...
> {
  ...
  let sym_is_nil =
    expr.alloc_equal(&mut cs.namespace(|| "sym is nil"), &g.nil_ptr)?;
  let sym_is_t =
  ...
  let env_is_nil =
    env.alloc_equal(&mut cs.namespace(|| "env is nil"), &g.nil_ptr)?;
  ...
  let var_or_rec_binding_is_sym =
    equal!(cs, var_or_rec_binding.tag(), &g.sym_tag)?;
  let var_or_rec_binding_is_cons =
    equal!(cs, var_or_rec_binding.tag(), &g.cons_tag)?;
...
> {
  ...
  macro_rules! def_head_sym {
    ($var:ident, $field:ident) => {
      let $var =
        head.alloc_equal(&mut cs.namespace(|| stringify!($var)), &g.$field)?;
    };
  }
  ...
  // - We only need to check the tag once, so can just check value/hash
  //   equality.
  ...
  let rest_is_nil =
    rest.alloc_equal(&mut cs.namespace(|| "rest_is_nil"), &g.nil_ptr)?;
  ...
  let more_is_nil =
    more.alloc_equal(&mut cs.namespace(|| "more_is_nil"), &g.nil_ptr)?;

  let arg1_is_cons =
    alloc_equal(&mut cs.namespace(|| "arg1_is_cons"), arg1.tag(), &g.cons_tag)?;
  let arg1_is_str =
    alloc_equal(&mut cs.namespace(|| "arg1_is_str"), arg1.tag(), &g.str_tag)?;

  let arg1_is_nil =
    arg1.alloc_equal(&mut cs.namespace(|| "arg1_is_nil"), &g.nil_ptr)?;
  ...
  let end_is_nil =
    more.alloc_equal(&mut cs.namespace(|| "end_is_nil"), &g.nil_ptr)?;
  ...
  let function = {
    ...
    let args_is_nil =
      args.alloc_equal(&mut cs.namespace(|| "args_is_nil"), &g.nil_ptr)?;

    let cdr_args_is_nil = cdr_args
      .alloc_equal(&mut cs.namespace(|| "cdr_args_is_nil"), &g.nil_ptr)?;

    let cdr_args_not_nil = cdr_args_is_nil.not();

    let lambda_not_dummy =
      and!(cs, &head_is_lambda, not_dummy, &cdr_args_not_nil)?;
  ...
  let bindings_is_nil = bindings
    .alloc_equal(&mut cs.namespace(|| "bindings_is_nil"), &g.nil_ptr)?;
  ...
  let body_is_nil =
    body.alloc_equal(&mut cs.namespace(|| "body_is_nil"), &g.nil_ptr)?;

  let rest_body_is_nil = rest_body
    .alloc_equal(&mut cs.namespace(|| "rest_body_is_nil"), &g.nil_ptr)?;
  ...
  let (
    ...
    let end_is_nil =
      end.alloc_equal(&mut cs.namespace(|| "end_is_nil"), &g.nil_ptr)?;

    // We get the condition for error by using OR of each individual error.
    let cond_error =
      or!(cs, &rest_body_is_nil.not(), &end_is_nil.not(), &body_is_nil)?;

    let rest_bindings_is_nil = rest_bindings
      .alloc_equal(&mut cs.namespace(|| "rest_bindings_is_nil"), &g.nil_ptr)?;

    let expanded_inner_not_dummy0 =
      and!(cs, &rest_bindings_is_nil.not(), &end_is_nil)?;
  ...
  let (
    ...
    // The following series of pick operations construct a preimage depending on
    // whether one or more arguments were provided to eval.
    //
    // If end_is_nil, then only one argument was provided, and the preimage is:
    // [unop_cont_tag, op1_eval_tag, cont.tag(), cont.hash(), g.default_num,
    // g.default_num, g.default_num, g.default_num]
    //
    // Otherwise, more than one argument was provided (FIXME: error-checking to
    // ensure exactly two were provided?): [binop_cont_tag, g.op2_eval_tag,
    // env.tag(), env.hash(), more.tag(), more.hash(), cont.tag(), cont.hash()]
  ...
  let (res, continuation) = {
    ...
    // NOTE: not_dummy was moved last to avoid namespace conflicts. Until
    // automatic-namespacing of and! and or! get smarter, put shared
    // elements last and unique ones first.
...
> {
  ...
  let (unop_val, unop_continuation) = {
    ...
    let res = multi_case(
      ...
      &[
        &[
          CaseClause { key: Op1::Car.as_field(), value: res_car.tag() },
          CaseClause { key: Op1::Cdr.as_field(), value: res_cdr.tag() },
          CaseClause { key: Op1::Atom.as_field(), value: atom_ptr.tag() },
          CaseClause { key: Op1::Emit.as_field(), value: result.tag() },
          CaseClause { key: Op1::Commit.as_field(), value: commitment.tag() },
          CaseClause { key: Op1::Open.as_field(), value: committed_expr.tag() },
          CaseClause { key: Op1::Secret.as_field(), value: &g.num_tag },
          CaseClause { key: Op1::Num.as_field(), value: num.tag() },
          CaseClause { key: Op1::U64.as_field(), value: &g.u64_tag },
          CaseClause { key: Op1::Comm.as_field(), value: comm.tag() },
          CaseClause { key: Op1::Char.as_field(), value: &g.char_tag },
          CaseClause { key: Op1::Eval.as_field(), value: result.tag() },
        ],
        &[
          CaseClause { key: Op1::Car.as_field(), value: allocated_car.hash() },
          CaseClause { key: Op1::Cdr.as_field(), value: allocated_cdr.hash() },
          CaseClause { key: Op1::Atom.as_field(), value: atom_ptr.hash() },
          CaseClause { key: Op1::Emit.as_field(), value: result.hash() },
          CaseClause { key: Op1::Commit.as_field(), value: commitment.hash() },
          CaseClause {
            key: Op1::Open.as_field(),
            value: committed_expr.hash(),
          },
          CaseClause { key: Op1::Secret.as_field(), value: &commitment_secret },
          CaseClause { key: Op1::Num.as_field(), value: num.hash() },
          CaseClause { key: Op1::U64.as_field(), value: &u64_elem },
          CaseClause { key: Op1::Comm.as_field(), value: comm.hash() },
          CaseClause { key: Op1::Char.as_field(), value: &u32_elem },
          CaseClause { key: Op1::Eval.as_field(), value: result.hash() },
        ],
      ],
      &[&g.default_num, &g.default_num],
      g,
  ...
  let (the_expr, the_env, the_cont, newer_cont2_not_dummy) = {
    ...
    let zero_arg_call_not_dummy =
      and!(cs, &call0_not_dummy, &body_t_is_cons, &args_is_dummy)?;
  ...
  let (the_expr, the_env, the_cont, newer_cont2_not_dummy) = {
...

    {
      ...
      let extend_not_dummy =
        and!(cs, &cont_is_call2_and_not_dummy, &args_is_dummy.not())?;
  ...
  let (the_expr, the_env, the_cont, newer_cont2_not_dummy) = {
    ...
    let rest_is_nil = allocated_rest
      .alloc_equal(&mut cs.namespace(|| "rest_is_nil"), &g.nil_ptr)?;
  ...
  let (the_expr, the_env, the_cont, make_thunk_num) = {
    ...
    let arg1_is_num =
      alloc_equal(&mut cs.namespace(|| "arg1_is_num"), arg1.tag(), &g.num_tag)?;
    let arg2_is_num =
      alloc_equal(&mut cs.namespace(|| "arg2_is_num"), arg2.tag(), &g.num_tag)?;
    ...
    let arg1_is_u64 =
      alloc_equal(&mut cs.namespace(|| "arg1_is_u64"), arg1.tag(), &g.u64_tag)?;
    let arg2_is_u64 =
      alloc_equal(&mut cs.namespace(|| "arg2_is_u64"), arg2.tag(), &g.u64_tag)?;
    ...
    let op2_is_mod =
      alloc_equal(cs.namespace(|| "op2_is_mod"), op2.tag(), &g.op2_modulo_tag)?;
    ...
    let quotient =
      constraints::div(&mut cs.namespace(|| "quotient"), a, &divisor)?;
    ...
    let arg2_is_str =
      alloc_equal(&mut cs.namespace(|| "arg2_is_str"), arg2.tag(), &g.str_tag)?;
    ...
    let val = case(
      ...
      op2.tag(),
      &[
        CaseClause { key: Op2::Sum.as_field(), value: &sum },
        CaseClause { key: Op2::Diff.as_field(), value: &diff },
        CaseClause { key: Op2::Product.as_field(), value: &product },
        CaseClause { key: Op2::Quotient.as_field(), value: &quotient },
        CaseClause { key: Op2::Equal.as_field(), value: args_equal_ptr.hash() },
        CaseClause {
          key: Op2::NumEqual.as_field(),
          value: args_equal_ptr.hash(),
        },
        CaseClause { key: Op2::Cons.as_field(), value: cons.hash() },
        CaseClause { key: Op2::StrCons.as_field(), value: cons.hash() },
        CaseClause { key: Op2::Hide.as_field(), value: commitment.hash() },
      ],
      &g.default_num,
      g,
    )?;
    ...
    // If the difference is negative, we need to add 2^64 to get back to U64
    // domain.
    ...
    let op2_is_diff =
      alloc_equal(cs.namespace(|| "op2_is_diff"), op2.tag(), &g.op2_diff_tag)?;
    ...
    let op2_is_eval =
      alloc_equal(cs.namespace(|| "op2_is_eval"), op2.tag(), &g.op2_eval_tag)?;
  ...
  let (res, the_cont) = {
    ...
    let condition_is_nil = condition
      .alloc_equal(&mut cs.namespace(|| "condition is nil"), &g.nil_ptr)?;
  ...
  let (body, extended_env, tail_cont, newer_cont2_not_dummy) = {
    ...
    let extended_env_not_dummy =
      and!(cs, &extended_env_not_dummy0, &cont_is_let)?;
  ...
  let (the_expr, the_env, the_cont, make_thunk_num, newer_cont2_not_dummy) = {
    ...
    let car_cdr_has_valid_tag =
      or!(cs, &result_has_cons_tag, &result_has_str_tag, &result_is_nil)?;
    ...
    let car_cdr_has_invalid_tag =
      and!(cs, &op1_is_car_or_cdr, &car_cdr_has_valid_tag.not())?;
  ...
  // This is all clunky because we can't currently return AllocatedBit from case
  // expressions.
  let newer_cont2_not_dummy_result_num = case_results[7].clone();
  let newer_cont2_not_dummy_ =
    equal!(cs, &newer_cont2_not_dummy_result_num, &g.true_num)?;
...
pub fn enforce_u64_div_mod<F: LurkField, CS: ConstraintSystem<F>>(
  ...
  let alloc_r_num = AllocatedNum::alloc(&mut cs.namespace(|| "r num"), || {
    Ok(F::from_u64(r).unwrap())
  })?;
  let alloc_q_num = AllocatedNum::alloc(&mut cs.namespace(|| "q num"), || {
...
fn car_cdr_named<F: LurkField, CS: ConstraintSystem<F>>(
    ...
    && !real_cons.get_value().unwrap_or(true)
  {
    ...
    panic!(
      "tried to take car_cdr of a non-dummy cons ({:?}) but supplied wrong \
       value",
      name
    );
  }
...
fn extend_rec<F: LurkField, CS: ConstraintSystem<F>>(
  ...
  let new_env_if_cons = AllocatedPtr::pick(
    ...
    &is_cons,
    &cons3,
    &g.error_ptr, /* This is being used as a signal, since extend_rec is
                   * expected to return a valid env. */
  )?;
...
mod tests {
  ...
  use super::*;
  use crate::{
    circuit::circuit_frame::constraints::{
      popcount,
      sub,
    },
    eval::{
  ...
  fn num_self_evaluating() {
    ...
    let public_params =
      Groth16Prover::create_groth_params(DEFAULT_CHUNK_FRAME_COUNT).unwrap();
