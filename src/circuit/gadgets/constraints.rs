...
/// Adds a constraint to CS, enforcing that the addition of the allocated
/// numbers in vector `v` is equal to `sum`.
///
/// summation(v) = sum
#[allow(dead_code)]
...
pub fn add_to_lc<F: PrimeField, CS: ConstraintSystem<F>>(
  ...
  match b {
    ...
    Boolean::Not(ref v) => {
      v_lc = v_lc + (scalar, CS::one()) - (scalar, v.get_variable())
    },
  };
...
// Enforce v is the bit decomposition of num, therefore we have that 0 <= num <
// 2ˆ(sizeof(v)).
...
pub fn div<F: PrimeField, CS: ConstraintSystem<F>>(
  ...
  let res = AllocatedNum::alloc(cs.namespace(|| "div_num"), || {
    let mut tmp = a.get_value().ok_or(SynthesisError::AssignmentMissing)?;
    let inv =
      (b.get_value().ok_or(SynthesisError::AssignmentMissing)?).invert();
...
/// Select the nth element of `from`, where `path_bits` represents n,
/// least-significant bit first. The returned result contains the selected
/// element, and constraints are enforced. `from.len()` must be a power of two.
#[allow(dead_code)]
pub fn select<F: PrimeField, CS: ConstraintSystem<F>>(
  ...
  // We reverse the path bits because the contained algorithm consumes most
...
pub fn alloc_equal<CS: ConstraintSystem<F>, F: PrimeField>(
  ...
  // Taken together, these constraints enforce that exactly one of `diff` and
  // `result` is 0. Since result is constrained to be boolean, that means
  // `result` is true iff `diff` is 0. `Diff` is 0 iff `a == b`.
...
pub fn alloc_is_zero<CS: ConstraintSystem<F>, F: PrimeField>(
  ...
  // Taken together, these constraints enforce that exactly one of `x` and
  // `result` is 0. Since result is constrained to be boolean, that means
  // `result` is true iff `x` is 0.
...
// field, then a modular reduction must have been carried out, changing the
// parity that should be even (since we multiplied by 2) to odd. In other words,
// we define negative numbers to be those field elements that are larger than
// p/2.
...
mod tests {
  ...
  fn add_constraint() {
    ...
    for _ in 0..100 {
      ...
      let a =
        AllocatedNum::alloc(cs.namespace(|| "a"), || Ok(Fr::random(&mut rng)))
          .expect("alloc failed");
      let b =
        AllocatedNum::alloc(cs.namespace(|| "b"), || Ok(Fr::random(&mut rng)))
          .expect("alloc failed");
  ...
  fn sub_constraint() {
    ...
    for _ in 0..100 {
      ...
      let res =
        sub(cs.namespace(|| "a-b"), &a, &b).expect("subtraction failed");
