...
macro_rules! if_then {
  ($cs:ident, $a:expr, $b:expr) => {
    enforce_implication(
      $cs
        .namespace(|| format!("if {} then {}", stringify!($a), stringify!($b))),
      $a,
      $b,
    )
  };
...
macro_rules! pick {
  ($cs:ident, $a:expr, $b:expr, $c:expr) => {{
    ...
    let c = $c;
    let cs = $cs.namespace(|| {
      format!("ifx {} {} {}", stringify!($a), stringify!($b), stringify!($c))
    });
    pick(cs, a, b, c)
  }};
}
...
macro_rules! pick_ptr {
  ($cs:ident, $a:expr, $b:expr, $c:expr) => {{
    ...
    let c = $c;
    let cs = $cs.namespace(|| {
      format!("ifx_t {} {} {}", stringify!($a), stringify!($b), stringify!($c))
    });
...
macro_rules! equal_t {
  ($cs:ident, $a:expr, $b:expr) => {
    $a.alloc_equal(
      $cs
        .namespace(|| format!("{} equal_t {}", stringify!($a), stringify!($b))),
      $b,
    )
  };
}
...
macro_rules! allocate_continuation_tag {
  ($cs:ident, $continuation_tag:expr) => {
    AllocatedNum::alloc(
      $cs.namespace(|| {
        format!("{} continuation tag", stringify!($continuation_tag))
      }),
