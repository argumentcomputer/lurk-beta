...
use crate::{
  ...
  store::{
    ContPtr, Continuation, Expression, IntoHashComponents, Ptr, ScalarContPtr,
...
use super::{
  constraints::{
    alloc_equal, alloc_equal_const, boolean_to_num, enforce_equal,
    enforce_implication, pick, pick_const,
  },
...
impl<F: LurkField> AllocatedPtr<F> {
    ...
    Fo: FnOnce() -> Result<ScalarPtr<F>, SynthesisError>,
  {
    ...
    Ok(AllocatedPtr { tag: alloc_tag, hash: alloc_hash })
  }
  ...
  pub fn alloc_constant<CS: ConstraintSystem<F>>(
    ...
    let alloc_tag =
      allocate_constant(&mut cs.namespace(|| "tag"), value.tag_field())?;
    let alloc_hash =
      allocate_constant(&mut cs.namespace(|| "hash"), *value.value())?;
    ...
    Fo: FnOnce() -> Result<&'a Ptr<F>, SynthesisError>,
  {
    AllocatedPtr::alloc(cs, || {
      let ptr = value()?;
      store.get_expr_hash(ptr).ok_or(SynthesisError::AssignmentMissing)
    })
  }
  ...
  pub fn alloc_constant_ptr<CS: ConstraintSystem<F>>(
    ...
    let ptr =
      store.get_expr_hash(value).ok_or(SynthesisError::AssignmentMissing)?;
    AllocatedPtr::alloc_constant(cs, ptr)
  }

  pub fn from_parts(tag: &AllocatedNum<F>, hash: &AllocatedNum<F>) -> Self {
    AllocatedPtr { tag: tag.clone(), hash: hash.clone() }
  }
  ...
  pub fn alloc_equal<CS: ConstraintSystem<F>>(
    ...
    let tags_equal =
      alloc_equal(&mut cs.namespace(|| "tags equal"), &self.tag, &other.tag)?;
  ...
  pub fn is_cons<CS: ConstraintSystem<F>>(
    &self,
    cs: &mut CS,
  ) -> Result<Boolean, SynthesisError> {
    self.alloc_tag_equal(
      &mut cs.namespace(|| "is_cons"),
      ExprTag::Cons.to_field(),
    )
  }
  pub fn is_str<CS: ConstraintSystem<F>>(
    &self,
    cs: &mut CS,
  ) -> Result<Boolean, SynthesisError> {
    self
      .alloc_tag_equal(&mut cs.namespace(|| "is_str"), ExprTag::Str.to_field())
  }
  pub fn is_num<CS: ConstraintSystem<F>>(
    &self,
    cs: &mut CS,
  ) -> Result<Boolean, SynthesisError> {
    self
      .alloc_tag_equal(&mut cs.namespace(|| "is_num"), ExprTag::Num.to_field())
  }
  pub fn is_u64<CS: ConstraintSystem<F>>(
    &self,
    cs: &mut CS,
  ) -> Result<Boolean, SynthesisError> {
    self
      .alloc_tag_equal(&mut cs.namespace(|| "is_u64"), ExprTag::U64.to_field())
  }
  pub fn is_char<CS: ConstraintSystem<F>>(
    &self,
    cs: &mut CS,
  ) -> Result<Boolean, SynthesisError> {
    self.alloc_tag_equal(
      &mut cs.namespace(|| "is_char"),
      ExprTag::Char.to_field(),
    )
  }
  pub fn is_comm<CS: ConstraintSystem<F>>(
    &self,
    cs: &mut CS,
  ) -> Result<Boolean, SynthesisError> {
    self.alloc_tag_equal(
      &mut cs.namespace(|| "is_comm"),
  ...
  pub fn is_sym<CS: ConstraintSystem<F>>(
    ...
    self
      .alloc_tag_equal(&mut cs.namespace(|| "is_sym"), ExprTag::Sym.to_field())
  }
  pub fn is_fun<CS: ConstraintSystem<F>>(
    &self,
    cs: &mut CS,
  ) -> Result<Boolean, SynthesisError> {
    self
      .alloc_tag_equal(&mut cs.namespace(|| "is_fun"), ExprTag::Fun.to_field())
  }
  pub fn is_thunk<CS: ConstraintSystem<F>>(
    ...
    self.alloc_tag_equal(
      &mut cs.namespace(|| "is_thunk"),
      ExprTag::Thunk.to_field(),
    )
  }
  ...
  pub fn alloc_hash_components<
    CS: ConstraintSystem<F>,
    T: IntoHashComponents<F>,
  >(
    cs: &mut CS,
    t: T,
  ) -> Result<Self, SynthesisError> {
  ...
  fn construct_cons<CS: ConstraintSystem<F>>(
    ...
    Ok(AllocatedPtr { tag: g.cons_tag.clone(), hash })
  }
  ...
  pub fn construct_cons_named<CS: ConstraintSystem<F>>(
    ...
    let real_car =
      car.alloc_equal(&mut cs.namespace(|| "real_car"), allocated_car)?;
    let real_cdr =
      cdr.alloc_equal(&mut cs.namespace(|| "real_cdr"), allocated_cdr)?;
    let cons_is_real = and!(cs, &real_car, &real_cdr)?;

    implies!(cs, not_dummy, &cons_is_real);

    if not_dummy.get_value().unwrap_or(false)
      && !cons_is_real.get_value().unwrap_or(true)
    {
      dbg!(name);
      panic!("uh oh!");
    }

    Ok(AllocatedPtr { tag: g.cons_tag.clone(), hash: allocated_digest.clone() })
  }
  ...
  pub fn construct_commitment<CS: ConstraintSystem<F>>(
    ...
    let preimage =
      vec![secret.clone(), expr.tag().clone(), expr.hash().clone()];
    ...
    Ok(AllocatedPtr { tag: g.comm_tag.clone(), hash })
  }
  ...
  pub fn construct_strcons<CS: ConstraintSystem<F>>(
    ...
    Ok(AllocatedPtr { tag: g.str_tag.clone(), hash })
  }
  ...
  pub fn construct_fun<CS: ConstraintSystem<F>>(
    ...
    Ok(AllocatedPtr { tag: g.fun_tag.clone(), hash })
  }
  ...
  pub fn construct_list<CS: ConstraintSystem<F>>(
    ...
    let tail = Self::construct_list(
      &mut cs.namespace(|| "Cons tail"),
      g,
      store,
      &elts[1..],
    )?;
  ...
  pub fn construct_thunk<CS: ConstraintSystem<F>>(
    ...
    Ok(AllocatedPtr { tag: g.thunk_tag.clone(), hash: thunk_hash })
  }
    ...
    CS: ConstraintSystem<F>,
  {
    ...
    let hash =
      pick_const(cs.namespace(|| "hash"), condition, *a.value(), *b.value())?;
...
impl<F: LurkField> AllocatedContPtr<F> {
    ...
    Fo: FnOnce() -> Result<ScalarContPtr<F>, SynthesisError>,
  {
    ...
    Ok(AllocatedContPtr { tag: alloc_tag, hash: alloc_hash })
  }
    ...
    Fo: FnOnce() -> Result<&'a ContPtr<F>, SynthesisError>,
  {
    AllocatedContPtr::alloc(cs, || {
      let ptr = value()?;
      store.hash_cont(ptr).ok_or(SynthesisError::AssignmentMissing)
    })
  }
  ...
  pub fn alloc_constant_cont_ptr<CS: ConstraintSystem<F>>(
    ...
    let ptr =
      store.hash_cont(value).ok_or(SynthesisError::AssignmentMissing)?;
  ...
  pub fn bind_input<CS: ConstraintSystem<F>>(
    ...
    let tag = AllocatedNum::alloc(cs.namespace(|| "continuation tag"), || {
      ptr.map(|c| c.tag_field()).ok_or(SynthesisError::AssignmentMissing)
    })?;
    tag.inputize(cs.namespace(|| "continuation tag input"))?;

    let hash =
      AllocatedNum::alloc(cs.namespace(|| "continuation hash"), || {
        ptr.map(|c| *c.value()).ok_or(SynthesisError::AssignmentMissing)
      })?;
  ...
  pub fn construct_named<CS: ConstraintSystem<F>>(
    ...
    for (i, (c1, c2)) in
      found_components.iter().zip(supplied_components).enumerate()
    {
      ...
      if let Some(a) = &acc {
        and!(
          &mut cs
            .namespace(|| format!("accumulate real component conjunction {i}")),
          a,
          &component_is_real
        )?;
      } else {
    ...
    let cont = AllocatedContPtr { tag: cont_tag.clone(), hash };
    Ok(cont)
  }
  ...
  pub fn construct_named_and_not_dummy<CS: ConstraintSystem<F>>(
    ...
    let (found_components, hash) =
      allocated_cont_witness.get_components_unconstrained(name);
  ...
  pub fn from_parts(tag: &AllocatedNum<F>, hash: &AllocatedNum<F>) -> Self {
    Self { tag: tag.clone(), hash: hash.clone() }
  }
}
