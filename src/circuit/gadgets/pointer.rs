...
impl<F: LurkField> AllocatedPtr<F> {
  ...
  pub fn alloc_constant<CS: ConstraintSystem<F>>(
    ...
    let alloc_tag =
      allocate_constant(&mut cs.namespace(|| "tag"), *value.tag())?;
    let alloc_hash =
  ...
  pub fn tag(&self) -> &AllocatedNum<F> { &self.tag }

  pub fn hash(&self) -> &AllocatedNum<F> { &self.hash }
  ...
  fn construct_cons<CS: ConstraintSystem<F>>(
    ...
    // This is actually binary_hash, considering creating that helper for use
    // elsewhere.
    let preimage = vec![
      car.tag().clone(),
      car.hash().clone(),
  ...
  /// Takes two allocated numbers (`a`, `b`) and returns `a` if the condition is
  ...
  pub fn bind_input<CS: ConstraintSystem<F>>(
    ...
    let tag = AllocatedNum::alloc(cs.namespace(|| "tag"), || {
      ptr.as_ref().map(|th| *th.tag()).ok_or(SynthesisError::AssignmentMissing)
    })?;
...
impl<F: LurkField> AllocatedContPtr<F> {
  ...
  pub fn bind_input<CS: ConstraintSystem<F>>(
    ...
    let tag = AllocatedNum::alloc(cs.namespace(|| "continuation tag"), || {
      ptr.map(|c| *c.tag()).ok_or(SynthesisError::AssignmentMissing)
    })?;
