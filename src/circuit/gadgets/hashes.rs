...
impl<Name: Debug, F: LurkField, PreimageType>
  Slot<Name, AllocatedHash<F, PreimageType>>
{
  pub fn new(name: Name, allocated: AllocatedHash<F, PreimageType>) -> Self {
    Self { name: Ok(name), allocated, consumed: false }
  }

  pub fn new_dummy(allocated: AllocatedHash<F, PreimageType>) -> Self {
    Self { name: Err(()), allocated, consumed: false }
  }

  #[allow(dead_code)]
  pub fn is_dummy(&self) -> bool { self.name.is_err() }

  pub fn is_blank(&self) -> bool { self.allocated.digest.get_value().is_none() }
...
impl<'a, F: LurkField> AllocatedConsWitness<'a, F> {
  pub fn from_cons_witness<CS: ConstraintSystem<F>>(
    ...
    for (i, (name, p)) in cons_witness.slots.iter().enumerate() {
      ...
      let allocated_car =
        AllocatedPtr::alloc(&mut cs.namespace(|| "car"), || {
          car_ptr.ok_or(SynthesisError::AssignmentMissing)
        })?;

      let allocated_cdr =
        AllocatedPtr::alloc(&mut cs.namespace(|| "cdr"), || {
    ...
    Ok(Self { witness: cons_witness, slots })
  }
  ...
  pub fn get_cons(
    ...
    let Slot { name: allocated_name, allocated: allocated_hash, consumed: _ } =
      &self.slots[index];
    if !expect_dummy {
...
impl<'a, F: LurkField> AllocatedContWitness<'a, F> {
  pub fn from_cont_witness<CS: ConstraintSystem<F>>(
    ...
    for (i, (name, p)) in cont_witness.slots.iter().enumerate() {
      ...
      let (cont_ptr, components) = match p {
        ...
        Stub::Value(cont) => {
          (Some(cont.cont_ptr), s.get_hash_components_cont(&cont.cont_ptr))
        },
      };
      ...
      let allocated_components = if let Some(components) = components {
        ...
        (0..8usize)
          .map(|i| {
            AllocatedNum::alloc(
              &mut cs.namespace(|| format!("component_{i}")),
              // This should never be called, because this branch is only taken
              // when stub is blank.
    ...
    Ok(Self { witness: cont_witness, slots })
  }
  ...
  pub fn get_components_unconstrained(
    ...
    // Debugging
    let _names_match =
      allocated_name.map(|alloc_name| alloc_name == name).unwrap_or(false);
