...
impl<F: LurkField> GlobalAllocations<F> {
  pub fn new<CS: ConstraintSystem<F>>(
    ...
    let error_ptr =
      AllocatedPtr::from_parts(error_ptr_cont.tag(), error_ptr_cont.hash());
    ...
    let nil_ptr = AllocatedPtr::alloc_constant_ptr(
      &mut cs.namespace(|| "nil"),
      store,
      &store.get_nil(),
    )?;
    ...
    let sym_tag =
      Tag::Sym.allocate_constant(&mut cs.namespace(|| "sym_tag"))?;
    let thunk_tag =
      Tag::Thunk.allocate_constant(&mut cs.namespace(|| "thunk_tag"))?;
    let cons_tag =
      Tag::Cons.allocate_constant(&mut cs.namespace(|| "cons_tag"))?;
    let char_tag =
      Tag::Char.allocate_constant(&mut cs.namespace(|| "char_tag"))?;
    let str_tag =
      Tag::Str.allocate_constant(&mut cs.namespace(|| "str_tag"))?;
    let num_tag =
      Tag::Num.allocate_constant(&mut cs.namespace(|| "num_tag"))?;
    let u64_tag =
      Tag::U64.allocate_constant(&mut cs.namespace(|| "u64_tag"))?;
    let comm_tag =
      Tag::Comm.allocate_constant(&mut cs.namespace(|| "comm_tag"))?;
    let fun_tag =
      Tag::Fun.allocate_constant(&mut cs.namespace(|| "fun_tag"))?;

    let outermost_cont_tag = ContTag::Outermost
      .allocate_constant(&mut cs.namespace(|| "outermost_cont_tag"))?;
    let lookup_cont_tag = ContTag::Lookup
      .allocate_constant(&mut cs.namespace(|| "lookup_cont_tag"))?;
    let let_cont_tag =
      ContTag::Let.allocate_constant(&mut cs.namespace(|| "let_cont_tag"))?;
    let letrec_cont_tag = ContTag::LetRec
      .allocate_constant(&mut cs.namespace(|| "letrec_cont_tag"))?;
    let tail_cont_tag =
      ContTag::Tail.allocate_constant(&mut cs.namespace(|| "tail_cont_tag"))?;
    let call0_cont_tag = ContTag::Call0
      .allocate_constant(&mut cs.namespace(|| "call0_cont_tag"))?;
    let call_cont_tag =
      ContTag::Call.allocate_constant(&mut cs.namespace(|| "call_cont_tag"))?;
    let call2_cont_tag = ContTag::Call2
      .allocate_constant(&mut cs.namespace(|| "call2_cont_tag"))?;
    let unop_cont_tag =
    ...
    let binop_cont_tag = ContTag::Binop
      .allocate_constant(&mut cs.namespace(|| "binop_cont_tag"))?;
    let binop2_cont_tag = ContTag::Binop2
      .allocate_constant(&mut cs.namespace(|| "binop2_cont_tag"))?;
    let if_cont_tag =
      ContTag::If.allocate_constant(&mut cs.namespace(|| "if_cont_tag"))?;

    let op1_car_tag =
      Op1::Car.allocate_constant(&mut cs.namespace(|| "op1_car_tag"))?;
    let op1_cdr_tag =
      Op1::Cdr.allocate_constant(&mut cs.namespace(|| "op1_cdr_tag"))?;
    let op1_commit_tag =
      Op1::Commit.allocate_constant(&mut cs.namespace(|| "op1_commit_tag"))?;
    let op1_num_tag =
      Op1::Num.allocate_constant(&mut cs.namespace(|| "op1_num_tag"))?;
    let op1_char_tag =
      Op1::Char.allocate_constant(&mut cs.namespace(|| "op1_char_tag"))?;
    let op1_eval_tag =
      Op1::Eval.allocate_constant(&mut cs.namespace(|| "op1_eval_tag"))?;
    let op1_u64_tag =
      Op1::U64.allocate_constant(&mut cs.namespace(|| "op1_u64_tag"))?;
    let op1_comm_tag =
      Op1::Comm.allocate_constant(&mut cs.namespace(|| "op1_comm_tag"))?;
    let op1_open_tag =
      Op1::Open.allocate_constant(&mut cs.namespace(|| "op1_open_tag"))?;
    let op1_secret_tag =
      Op1::Secret.allocate_constant(&mut cs.namespace(|| "op1_secret_tag"))?;
    let op1_atom_tag =
      Op1::Atom.allocate_constant(&mut cs.namespace(|| "op1_atom_tag"))?;
    let op1_emit_tag =
      Op1::Emit.allocate_constant(&mut cs.namespace(|| "op1_emit_tag"))?;
    let op2_eval_tag =
      Op2::Eval.allocate_constant(&mut cs.namespace(|| "op2_eval_tag"))?;
    let op2_cons_tag =
      Op2::Cons.allocate_constant(&mut cs.namespace(|| "op2_cons_tag"))?;
    let op2_strcons_tag = Op2::StrCons
      .allocate_constant(&mut cs.namespace(|| "op2_strcons_tag"))?;
    let op2_hide_tag =
      Op2::Hide.allocate_constant(&mut cs.namespace(|| "op2_hide_tag"))?;
    let op2_begin_tag =
      Op2::Begin.allocate_constant(&mut cs.namespace(|| "op2_begin_tag"))?;
    let op2_sum_tag =
      Op2::Sum.allocate_constant(&mut cs.namespace(|| "op2_sum_tag"))?;
    let op2_diff_tag =
      Op2::Diff.allocate_constant(&mut cs.namespace(|| "op2_diff_tag"))?;

    let op2_product_tag = Op2::Product
      .allocate_constant(&mut cs.namespace(|| "op2_product_tag"))?;
    let op2_quotient_tag = Op2::Quotient
      .allocate_constant(&mut cs.namespace(|| "op2_quotient_tag"))?;
    let op2_modulo_tag =
    ...
    let op2_less_equal_tag = Op2::LessEqual
      .allocate_constant(&mut cs.namespace(|| "op2_less_equal_tag"))?;
    let op2_greater_tag = Op2::Greater
      .allocate_constant(&mut cs.namespace(|| "op2_greater_tag"))?;
    let op2_greater_equal_tag = Op2::GreaterEqual
      .allocate_constant(&mut cs.namespace(|| "op2_greater_equal_tag"))?;
    let op2_equal_tag =
      AllocatedNum::alloc(&mut cs.namespace(|| "op2_equal_tag"), || {
    ...
    let false_num =
      allocate_constant(&mut cs.namespace(|| "false"), F::zero())?;
    let default_num =
      allocate_constant(&mut cs.namespace(|| "default"), F::zero())?;

    let power2_32_ff = F::pow_vartime(&F::from_u64(2).unwrap(), [32]);
    let power2_32_num =
      allocate_constant(&mut cs.namespace(|| "pow(2,32)"), power2_32_ff)?;

    let power2_64_ff = F::pow_vartime(&F::from_u64(2).unwrap(), [64]);
    let power2_64_num =
      allocate_constant(&mut cs.namespace(|| "pow(2,64)"), power2_64_ff)?;
...
impl<F: LurkField> Ptr<F> {
  pub fn allocate_maybe_fun_unconstrained<CS: ConstraintSystem<F>>(
    ...
    (AllocatedNum<F>, AllocatedPtr<F>, AllocatedPtr<F>, AllocatedPtr<F>),
    SynthesisError,
  > {
    match maybe_fun.map(|ptr| (ptr, ptr.tag())) {
      Some((ptr, Tag::Fun)) => match store.fetch(ptr).expect("missing fun") {
        Expression::Fun(arg, body, closed_env) => {
          ...
          let closed_env =
            store.get_expr_hash(&closed_env).expect("missing closed env");
  ...
  fn allocate_fun<CS: ConstraintSystem<F>, T: IntoHashComponents<F>>(
    ...
    SynthesisError,
  > {
    let arg_t = AllocatedPtr::alloc_hash_components(
      &mut cs.namespace(|| "allocate arg tag"),
      arg,
    )?;
    let body_t = AllocatedPtr::alloc_hash_components(
      &mut cs.namespace(|| "allocate body tag"),
      body,
    )?;
...
pub fn allocate_constant<F: LurkField, CS: ConstraintSystem<F>>(
  ...
  let allocated =
    AllocatedNum::<F>::alloc(cs.namespace(|| "allocate"), || Ok(val))?;
...
impl<F: LurkField> Thunk<F> {
  pub fn allocate_maybe_dummy_components<CS: ConstraintSystem<F>>(
    ...
    (AllocatedNum<F>, AllocatedPtr<F>, AllocatedContPtr<F>),
    SynthesisError,
  > {
  ...
  > {
    ...
    let hash =
      Self::hash_components(cs.namespace(|| "Thunk"), store, &value, &cont)?;
  ...
  pub fn hash_components<CS: ConstraintSystem<F>>(
    ...
    // This is a 'binary' hash but has arity 4 because of tag and hash
    // components for each item.
    let hash = hash_poseidon(
      cs.namespace(|| "Thunk Continuation"),
      vec![vs[0].clone(), vs[1].clone(), conts[0].clone(), conts[1].clone()],
