...
impl<F: LurkField> GlobalAllocations<F> {
  pub fn new<CS: ConstraintSystem<F>>(
    ...
    let thunk_tag =
      ExprTag::Thunk.allocate_constant(&mut cs.namespace(|| "thunk_tag"))?;
    let cons_tag =
      ExprTag::Cons.allocate_constant(&mut cs.namespace(|| "cons_tag"))?;
    let char_tag =
      ExprTag::Char.allocate_constant(&mut cs.namespace(|| "char_tag"))?;
    let str_tag =
      ExprTag::Str.allocate_constant(&mut cs.namespace(|| "str_tag"))?;
    let num_tag =
      ExprTag::Num.allocate_constant(&mut cs.namespace(|| "num_tag"))?;
    let u64_tag =
      ExprTag::U64.allocate_constant(&mut cs.namespace(|| "u64_tag"))?;
    let comm_tag =
      ExprTag::Comm.allocate_constant(&mut cs.namespace(|| "comm_tag"))?;
    let fun_tag =
      ExprTag::Fun.allocate_constant(&mut cs.namespace(|| "fun_tag"))?;
