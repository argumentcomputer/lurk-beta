...
impl<F: LurkField> CaseConstraint<'_, F> {
  ...
  fn enforce_selection<CS: ConstraintSystem<F>>(
    ...
    for (i, clause) in self.clauses.iter().enumerate() {
      ...
      debug_assert!(
        self.clauses.iter().filter(|c| c.key == clause.key).count() == 1
      );
    ...
    cs.enforce(
      ...
      |lc| {
        selector.iter().fold(lc, |lc, selector| lc + selector.get_variable())
      },
      |lc| lc + CS::one(),
      |lc| lc + CS::one(),
    );
    ...
    cs.enforce(
      ...
      |lc| {
        selector.iter().zip(self.clauses).fold(lc, |lc, (selector, clause)| {
    ...
    let values =
      self.clauses.iter().map(|c| c.value.clone()).collect::<Vec<_>>();
...
fn selector_dot_product<F: LurkField, CS: ConstraintSystem<F>>(
  ...
  let result =
    AllocatedNum::<F>::alloc(&mut cs.namespace(|| "result"), || {
      Ok(computed_result)
    })?;

  cs.enforce(
    || "sum of products",
    |lc| all_products.iter().fold(lc, |acc, prod| acc + prod.get_variable()),
    |lc| lc + CS::one(),
  ...
  let (selector, is_default) = {
    ...
    for (i, clause) in c.iter().enumerate() {
      ...
      cs.enforce(
        ...
        |_| {
          Boolean::Constant(true).lc(CS::one(), clause.key)
            - selected.get_variable()
        },
    ...
    let is_selected =
      alloc_is_zero(cs.namespace(|| format!("is_selected, case {i}")), &acc)?;
    ...
    let selected_key =
      AllocatedNum::alloc(cs.namespace(|| "default key"), || {
        if any_selected {
          selected.get_value().ok_or(SynthesisError::AssignmentMissing)
        }
        else {
          Ok(dummy_key)
        }
    ...
    let cc = CaseConstraint { selected_key, clauses: c };
  ...
  for (i, (c, default)) in cases.iter().zip(defaults).enumerate().skip(1) {
    ...
    for (j, case) in c.iter().enumerate() {
      debug_assert_eq!(
        ...
        "Key ordering missmatch {i}:{j}"
      );
    }

    let values =
      c.iter().map(|clause| clause.value.clone()).collect::<Vec<_>>();
...
mod tests {
  ...
  fn simple_case() {
    ...
    let y = Fr::from(124);
    let selected =
      AllocatedNum::alloc(cs.namespace(|| "selected"), || Ok(x)).unwrap();
    let val0 =
      AllocatedNum::alloc(cs.namespace(|| "val0"), || Ok(Fr::from(666)))
        .unwrap();
    let val1 =
      AllocatedNum::alloc(cs.namespace(|| "val1"), || Ok(Fr::from(777)))
        .unwrap();
    let default =
      AllocatedNum::alloc(cs.namespace(|| "default_v"), || Ok(Fr::from(999)))
        .unwrap();

    {
      let clauses = [
        CaseClause { key: x, value: &val0 },
        CaseClause { key: y, value: &val1 },
      ];
    ...
    {
      let clauses = [CaseClause { key: y, value: &val0 }];

      let default_chosen =
        AllocatedNum::alloc(cs.namespace(|| "default chosen"), || {
          Ok(Fr::from(999))
        })
        .unwrap();
      let result = case(
  ...
  fn multicase() {
    ...
    let z = Fr::from(125);
    let selected0 =
      AllocatedNum::alloc(cs.namespace(|| "selected0"), || Ok(x)).unwrap();
    let selected1 =
      AllocatedNum::alloc(cs.namespace(|| "selected1"), || Ok(z)).unwrap();
    let val0 =
      AllocatedNum::alloc(cs.namespace(|| "val0"), || Ok(Fr::from(666)))
        .unwrap();
    let val1 =
      AllocatedNum::alloc(cs.namespace(|| "val1"), || Ok(Fr::from(777)))
        .unwrap();
    let val2 =
      AllocatedNum::alloc(cs.namespace(|| "val2"), || Ok(Fr::from(700)))
        .unwrap();
    let default_vec = [
      &AllocatedNum::alloc(cs.namespace(|| "default0"), || Ok(Fr::from(999)))
        .unwrap(),
      &AllocatedNum::alloc(cs.namespace(|| "default1"), || Ok(Fr::from(998)))
        .unwrap(),
      &AllocatedNum::alloc(cs.namespace(|| "default2"), || Ok(Fr::from(997)))
        .unwrap(),
    ];

    {
      ...
      let clauses1: [CaseClause<Fr>; 2] = [
        CaseClause { key: x, value: &val1 },
        CaseClause { key: y, value: &val0 },
      ];
      let clauses2: [CaseClause<Fr>; 2] = [
        CaseClause { key: x, value: &val2 },
        CaseClause { key: y, value: &val0 },
      ];
      let clauses_vec: [&[CaseClause<Fr>]; 3] =
        [&clauses0, &clauses1, &clauses2];
  ...
  #[should_panic]
  fn multicase_negative1_panic() { multicase_negative1_aux(); }

  #[test]
  #[cfg(not(debug_assertions))]
  fn multicase_negative1_unsatisfied() { multicase_negative1_aux(); }
  ...
  fn multicase_negative1_aux() {
    ...
    let z = Fr::from(125);
    let selected =
      AllocatedNum::alloc(cs.namespace(|| "selected"), || Ok(z)).unwrap();
    let val0 =
      AllocatedNum::alloc(cs.namespace(|| "val0"), || Ok(Fr::from(666)))
        .unwrap();
    let val1 =
  ...
  fn groth_case() {
    ...
    let selected =
      AllocatedNum::alloc(cs.namespace(|| "selected"), || Ok(x)).unwrap();
    let _selected_blank =
      AllocatedNum::alloc(cs_blank.namespace(|| "selected"), || {
        Err(SynthesisError::AssignmentMissing)
      })
      .unwrap();
    let val0 =
    ...
    let default =
      AllocatedNum::alloc(cs.namespace(|| "default_v"), || Ok(Fr::from(999)))
        .unwrap();
    let default_blank =
      AllocatedNum::alloc(cs_blank.namespace(|| "default_v"), || {
        Err(SynthesisError::AssignmentMissing)
      })
      .unwrap();
    ...
    {
      let clauses_blank = [
        CaseClause { key: x, value: &val0_blank },
        CaseClause { key: y, value: &val1_blank },
      ];
