...
pub trait LurkField: PrimeField + PrimeFieldBits {
  ...
  // Return a u32 corresponding to the first 4 little-endian bytes of this field
  ...
  fn from_multicodec(codec: u64) -> Option<Self> {
    ...
    if lurk_prefix != Self::LURK_CODEC_PREFIX
      || field_prefix != Self::FIELD_CODEC
    {
      None
    }
    else {
  ...
  where D: serde::Deserializer<'de> {
    ...
    let f = F::from_bytes(&bytes).ok_or_else(|| {
      D::Error::custom(format!(
        "expected field element as bytes, got {:?}",
        &bytes
      ))
    })?;
    Ok(FWrap(f))
