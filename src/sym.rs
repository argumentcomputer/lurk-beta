...
use crate::parser::{
  maybe_quote_symbol_name_string,
  read_symbol_path,
  KEYWORD_MARKER,
  SYM_SEPARATOR,
};
...
#[derive(
  Clone, Debug, Eq, PartialEq, PartialOrd, Serialize, Deserialize, Hash,
)]
pub struct Symbol {
  pub path: Vec<String>,
  // It would be better not to have this here, but it simplifies things in the
...
impl Symbol {
  pub fn root() -> Self { Self::new_from_path(vec!["".into()]) }

  pub fn new(name: String) -> Self {
    let path = Self::path_from_name(&name);
    Self { path, opaque: false }
  }
  ...
  pub fn full_name(&self) -> String { Self::name_from_path(&self.path) }
  ...
  pub fn new_opaque() -> Self {
    Symbol { path: Default::default(), opaque: true }
  }

  pub fn path(&self) -> &Vec<String> { &self.path }

  fn new_from_path(path: Vec<String>) -> Self { Symbol { path, opaque: false } }

  // This is the function that normalizes full names by converting all path
  // segments to their quoted forms if and only if needed.
...
impl Sym {
  ...
  pub fn new(name: String) -> Self {
    ...
    match chars.next() {
      Some(c) if c == KEYWORD_MARKER => {
        Self::Key(Symbol::new(chars.collect::<String>()))
      },
  ...
  pub fn root() -> Self { Self::Sym(Symbol::root()) }

  pub fn key_root() -> Self { Self::Key(Symbol::root()) }
  ...
  // This is just the name part (i.e. last path segment) of the symbol or
  // keyword.
  pub fn name(&self) -> String {
    match self {
      Self::Sym(s) => s.name(),
  ...
  pub fn is_keyword(&self) -> bool { matches!(self, Self::Key(_)) }

  pub fn is_toplevel(&self) -> bool { self.symbol().is_toplevel() }

  pub fn is_root(&self) -> bool { self.symbol().is_root() }
...
impl From<&str> for Sym {
  fn from(s: &str) -> Sym { Sym::new(s.into()) }
}

impl From<String> for Sym {
  fn from(s: String) -> Sym { Sym::new(s) }
}
