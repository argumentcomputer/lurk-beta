  ...
  fn fmt<W: io::Write>(&self, store: &Store<F>, w: &mut W) -> io::Result<()> {
    ...
    if self.is_opaque() {
      ...
      if let Some(x) = store.get_expr_hash(self) {
        write!(w, " ")?;
        crate::store::Expression::Num(crate::num::Num::Scalar(*x.value()))
          .fmt(store, w)?;
      }
      write!(w, ">")
    }
  ...
  fn fmt<W: io::Write>(&self, store: &Store<F>, w: &mut W) -> io::Result<()> {
    ...
    match self {
      ...
      Fun(arg, body, _closed_env) => {
        let is_zero_arg =
          *arg == store.get_lurk_sym("_", true).expect("dummy_arg (_) missing");
      ...
      Comm(secret, payload) => {
        ...
        write!(w, "(comm ")?;
        let c =
          store.commitment_hash(*secret, store.get_expr_hash(payload).unwrap());
