...
pub(super) struct MetaCmd<F: LurkField>
{
    ...
    pub(super) run: fn(repl: &mut Repl<F>, cmd: &str, args: &Ptr<F>, pwd_path: &Utf8Path) -> Result<()>,
}
...
impl MetaCmd<F> {
    const LOAD: MetaCmd<F> = MetaCmd {
        name:
            "load",
        summary:
            "Load lurk expressions from a file path.",
        format:
            "!(load <string>)",
        description: &[],
        example: &[
...
impl MetaCmd<F> {
    const DEFREC: MetaCmd<F> = MetaCmd {
        name:
            "defrec",
        summary:
            "Extends the env with a recursive binding.",
        format:
            "!(defrec <binding> <body>)",
        description: &[
...
impl MetaCmd<F> {
    const ASSERT_EQ: MetaCmd<F> = MetaCmd {
        name:
            "assert-eq",
        summary:
            "Assert that two expressions evaluate to the same value.",
        format:
            "!(assert-eq <expr> <expr>)",
        description: &[],
        example: &[
...
impl MetaCmd<F> {
    const ASSERT_ERROR: MetaCmd<F> = MetaCmd {
        name:
            "assert-error",
        summary:
            "Assert that a evaluation of <expr> fails.",
        format:
            "!(assert-error <expr>)",
        description: &[],
        example: &[
...
impl MetaCmd<F> {
    const HIDE: MetaCmd<F> = MetaCmd {
        name:
            "hide",
        summary:
            "Return and persist the commitment of <exp> using secret <secret>.",
        format:
            "!(hide <secret> <expr>)",
        description: &[],
        example: &[
...
impl MetaCmd<F> {
    const FETCH: MetaCmd<F> = MetaCmd {
        name:
            "fetch",
        summary:
            "Add data from a commitment to the repl store.",
        format:
            "!(fetch <commitment>)",
        description: &[],
        example: &[
...
impl<F: LurkField> MetaCmd<F> {
    const CLEAR: MetaCmd<F> = MetaCmd {
        name:
            "clear",
        summary:
            "Reset the current environment to be empty.",
        format:
            "!(clear)",
        description: &[],
        example: &[
...
impl MetaCmd<F> {
    const SET_ENV: MetaCmd<F> = MetaCmd {
        name:
            "set-env",
        summary:
            "Set the env to the result of evaluating the first argument.",
        format:
            "!(set-env <expr>)",
        description: &[],
        example: &[
...
impl<F: LurkField> MetaCmd<F> {
    const IMPORT: MetaCmd<F> = MetaCmd {
        name:
            "import",
        summary:
            "Import a single or several packages.",
        format:
