...
impl<F: LurkField, T: ReplTrait<F>> Repl<F, T> {
  pub fn new(
    ...
    Ok(Self { state, rl, history_path, _phantom: Default::default() })
  }
...
impl<F: LurkField> ReplState<F> {
  pub fn new(s: &mut Store<F>, limit: usize, command: Option<Command>) -> Self {
    Self { env: empty_sym_env(s), limit, command }
  }
  pub fn eval_expr(
    &mut self,
    expr: Ptr<F>,
    ...
    let IO { expr: result, env: _env, cont: next_cont } = io;
  ...
  pub fn handle_file<P: AsRef<Path> + Copy>(
    ...
    eprintln!("Read from {}: {}", file_path.as_ref().to_str().unwrap(), input);
          ...
          if let Some(name) = s.simple_keyword_name() {
            match name.as_str() {
              ...
              "DEF" => {
                ...
                let (expanded_evaled, _, _, _) =
                  self.eval_expr(expanded, store)?;
              ...
              "DEFREC" => {
                ...
                let (new_binding_inner, _) =
                  store.car_cdr(&new_binding_outer)?;
  ...
  fn handle_non_meta(
    ...
    match Evaluator::new(expr_ptr, self.env, store, self.limit).eval() {
      Ok((output, iterations, _emitted)) => {
        let IO { expr: result, env: _env, cont: next_cont } = output;
        {
          if !update_env {
