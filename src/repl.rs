...
impl<F: LurkField> Repl<F> {
  pub fn new(s: &mut Store<F>, limit: usize) -> Result<Self> {
    let history_path =
      dirs::home_dir().expect("missing home directory").join(".lurk-history");

    let h = InputValidator { brackets: MatchingBracketValidator::new() };
...
impl<F: LurkField> ReplState<F> {
  pub fn new(s: &mut Store<F>, limit: usize) -> Self {
    Self { env: empty_sym_env(s), limit }
  }
  ...
  pub fn eval_expr(
    ...
    let (IO { expr: result, env: _env, cont: next_cont }, iterations, emitted) =
      Evaluator::new(expr, self.env, store, self.limit).eval().unwrap();
  ...
  pub fn maybe_handle_command(
    ...
    let result = match &maybe_command {
      Ok(maybe_command) => match maybe_command.tag() {
        Tag::Sym => {
          if let Some(key_string) =
            store.fetch(maybe_command).unwrap().as_simple_keyword_string()
          {
  ...
  pub fn handle_file<P: AsRef<Path> + Copy>(
    ...
    println!("Read from {}: {}", file_path.as_ref().to_str().unwrap(), input);
  ...
  fn handle_non_meta(
    ...
    match Evaluator::new(expr_ptr, self.env, store, self.limit).eval() {
      Ok((
        IO { expr: result, env: _env, cont: next_cont },
        iterations,
        _emitted,
      )) => {
        if !update_env {
        ...
        match next_cont.tag() {
          ContTag::Outermost | ContTag::Terminal => {
            ...
            // We are either loading/running and update the env, or evaluating
            // and don't.
            if update_env {
              self.env = result
            }
        ...
        Expression::Sym(s) => {
          if let Some(name) = s.simple_keyword_name() {
            if &name == "LOAD" {
              ...
              // Running and loading are equivalent, except that :RUN does not
              // modify the env.
              match store.fetch(&store.car(&rest)?).unwrap() {
                Expression::Str(path) => {
                  let joined = p.as_ref().join(Path::new(&path));
              ...
              assert!(rest.is_nil());
              let (first_evaled, ..) = self.eval_expr(first, store);
              let (second_evaled, ..) = self.eval_expr(second, store);
              assert!(
              ...
              assert!(rest.is_nil());
              let (first_evaled, ..) = self.clone().eval_expr(first, store);
              ...
              for (i, elem) in emitted.iter().enumerate() {
                if elem != &first_emitted {
                  panic!(
                    ":ASSERT-EMITTED failed at position {}. Expected {}, but \
                     found {}.",
                    i,
