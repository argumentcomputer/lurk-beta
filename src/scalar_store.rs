  ...
  fn from_ptr(store: &Store<F>, ptr: &Ptr<F>) -> Option<Self> {
    match ptr.tag() {
      ...
      ExprTag::Cons => store.fetch_cons(ptr).and_then(|(car, cdr)| {
        if let (Some(car), Some(cdr)) =
          (store.get_expr_hash(car), store.get_expr_hash(cdr))
        {
      ...
      ExprTag::Sym | ExprTag::Key => {
        store.fetch_sym(ptr).map(ScalarExpression::Sym)
      },
      ExprTag::Fun => {
        store.fetch_fun(ptr).and_then(|(arg, body, closed_env)| {
          if let (Some(arg), Some(body), Some(closed_env)) = (
            store.get_expr_hash(arg),
            store.get_expr_hash(body),
            store.get_expr_hash(closed_env),
          ) {
            Some(ScalarExpression::Fun { arg, body, closed_env })
          } else {
            None
          }
        })
...
mod test {
  ...
  // testing ipld
  fn ipld_scalar_store_strategy<Fr: LurkField>(
  ) -> impl Strategy<Value = ScalarStore<Fr>> {
    (
