...
/// `ScalarStore` allows realization of a graph of `ScalarPtr`s suitable for
/// serialization to IPLD. `ScalarExpression`s are composed only of
/// `ScalarPtr`s, so `scalar_map` suffices to allow traverseing an arbitrary
/// DAG.
...
impl<F: LurkField> ScalarStore<F> {
  /// Create a new `ScalarStore` and add all `ScalarPtr`s reachable in the
  /// scalar representation of `expr`.
  ...
  /// Add the `ScalarPtr` and `ScalarExpression` associated with `ptr`. The
  /// relationship between `ptr` and `scalar_ptr` is not checked here, so
  /// `add` should only be called by `add_ptr` and `add_scalar_ptr`, which
  ...
  fn add(&mut self, store: &Store<F>, ptr: &Ptr<F>, scalar_ptr: ScalarPtr<F>) {
    ...
    // If `scalar_ptr` is not already in the map, queue its children for
    // processing.
    self.scalar_map.entry(scalar_ptr).or_insert_with(|| {
      let scalar_expression = ScalarExpression::from_ptr(store, ptr)?;
      if let Some(more_scalar_ptrs) =
        Self::child_scalar_ptrs(&scalar_expression)
      {
  ...
  fn child_scalar_ptrs(
    scalar_expression: &ScalarExpression<F>,
  ) -> Option<Vec<ScalarPtr<F>>> {
  ...
  /// Unqueue all the pending `ScalarPtr`s and add them, queueing all of their
  /// children, then repeat until the queue is pending queue is empty.
  ...
  /// Method which finalizes the `ScalarStore`, ensuring that all reachable
  /// `ScalarPtr`s have been added.
  ...
  pub fn get_cont(
    &self,
    ptr: &ScalarContPtr<F>,
  ) -> Option<&ScalarContinuation<F>> {
    let x = self.scalar_cont_map.get(ptr)?;
    (*x).as_ref()
  }

  pub fn to_store_with_expr(
    &mut self,
    ptr: &ScalarPtr<F>,
  ) -> Option<(Store<F>, Ptr<F>)> {
    ...
    match ptr.tag() {
      ...
      Tag::Cons => store.fetch_cons(ptr).and_then(|(car, cdr)| {
        store.get_expr_hash(car).and_then(|car| {
          store.get_expr_hash(cdr).map(|cdr| ScalarExpression::Cons(car, cdr))
        })
      }),
      ...
      Tag::Str => {
        store.fetch_str(ptr).map(|str| ScalarExpression::Str(str.to_string()))
      },
...
pub enum ScalarExpression<F: LurkField> {
  ...
  Comm(F, ScalarPtr<F>),
  Sym(Sym),
  Fun { arg: ScalarPtr<F>, body: ScalarPtr<F>, closed_env: ScalarPtr<F> },
  Num(F),
  Str(String),
  Thunk(ScalarThunk<F>),
  Char(char),
...
mod test {
  ...
  impl Arbitrary for Symbol {
    fn arbitrary(g: &mut Gen) -> Self {
      Symbol { path: Arbitrary::arbitrary(g), opaque: Arbitrary::arbitrary(g) }
    }
  }
    ...
    fn arbitrary(g: &mut Gen) -> Self {
      let input: Vec<(i64, Box<dyn Fn(&mut Gen) -> ScalarExpression<Fr>>)> = vec![
        ...
        (
          100,
          Box::new(|g| {
            Self::Cons(ScalarPtr::arbitrary(g), ScalarPtr::arbitrary(g))
          }),
        ),
  ...
  impl Arbitrary for ScalarStore<Fr> {
    fn arbitrary(g: &mut Gen) -> Self {
      let map: Vec<(ScalarPtr<Fr>, Option<ScalarExpression<Fr>>)> =
  ...
  fn test_expr_ipld() {
    let test = |src| {
      ...
      if let (scalar_store, Some(scalar_expr)) =
        ScalarStore::new_with_expr(&store1, &expr1)
      {
        ...
        if let Some((mut store2, expr2)) =
          scalar_store2.to_store_with_expr(&scalar_expr)
        {
  ...
  fn test_expr_eval_ipld() {
    ...
    let test = |src| {
      ...
      let (eval::IO { expr, env: _, cont: _ }, _lim, _emitted) =
        eval.eval().unwrap();
  ...
  fn test_scalar_store_opaque_cons() {
    ...
    // If a non-opaque version has been found when interning opaque, children
    // appear in `ScalarStore`.
  ...
  fn test_scalar_store_opaque_sym() {
    ...
    // Only the symbol's string itself, not all of its substrings, appears in
    // `ScalarStore`.
  ...
  fn test_scalar_store_opaque_str() {
    ...
    // Only the string itself, not all of its substrings, appears in
    // `ScalarStore`.
    assert_eq!(1, scalar_store.scalar_map.len());
  }
  #[test]
