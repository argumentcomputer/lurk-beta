...
impl<T> Stub<T> {
  fn is_dummy(&self) -> bool { matches!(self, Self::Dummy) }
}
...
impl<F: LurkField> ConsStub<F> {
  pub fn car_cdr(
    ...
    match self {
      ...
      Self::Blank => {
        unreachable!("Blank ConsStub should be used only in blank circuits.")
      },
...
impl<Name: HashName, T, const L: usize, F: LurkField>
  HashWitness<Name, T, L, F>
{
  pub fn length() -> usize { L }
}

pub type ConsWitness<F> =
  HashWitness<ConsName, Cons<F>, MAX_CONSES_PER_REDUCTION, F>;
pub type ContWitness<F> =
  HashWitness<ContName, Cont<F>, MAX_CONTS_PER_REDUCTION, F>;

impl<F: LurkField> HashWitness<ConsName, Cons<F>, MAX_CONSES_PER_REDUCTION, F> {
  #[allow(dead_code)]
  fn assert_specific_invariants(&self, store: &Store<F>) {
    // Use the commented code below to search for (non-nil) duplicated conses,
    // which could indicate that two different Cons are being used to
    // reference the identical structural value. In that case, they could be
    // coalesced, potentially leading to fewer slots being required.
    //
    // As of the initial optimization pass, Env and ExtendedClosureEnv appear to
    // be duplicated in this way. However, it's not clear why that is, and
    // coalescing them does not obviously lead to a potential savings, so we
    // will leave them distinct for now.
  ...
  > HashWitness<Name, T, MAX_T_PER_REDUCTION, F>
{
  ...
  pub fn new_dummy() -> Self { Self::new_from_stub(Stub::Dummy) }

  pub fn new_blank() -> Self { Self::new_from_stub(Stub::Blank) }
  ...
  pub fn get_assigned_slot(&mut self, name: Name) -> &mut Stub<T> {
    ...
    if slot_name == name {
      ...
      panic!("double booked: found {:?} when getting {:?}", &slot_name, &name);
    }
  }
  ...
  pub fn stubs_used(&self) -> Vec<Stub<T>> {
    self.all_stubs().into_iter().filter(|c| !c.is_dummy()).collect()
  }
  ...
  pub fn total_stub(&self) -> usize { self.all_stubs().len() }
}
...
impl<F: LurkField> ContWitness<F> {
  ...
  pub fn intern_named_cont(
    ...
    self.get_assigned_slot(name).intern_cont(store, continuation)
  }
}
  ...
  pub fn fetch_cont(
    ...
    match self {
      Self::Dummy => {
        ...
        *self = Self::Value(Cont { cont_ptr: *cont, continuation });

        Some(continuation)
      },
      Self::Blank => {
        unreachable!("Blank ContStub should be used only in blank circuits.")
      },
  ...
  pub fn intern_cont(
    ...
    match self {
      Self::Dummy => {
        ...
        *self = Self::Value(Cont { cont_ptr, continuation });
        cont_ptr
      },
      Self::Blank => {
        unreachable!("Blank ContStub should be used only in blank circuits.")
