...
pub fn eval_step() -> &'static Func {
    EVAL_STEP.get_or_init(|| make_eval_step(&[]))
}
...
pub fn make_eval_step_from_lang<F: LurkField, C: Coprocessor<F>>(lang: &Lang<F, C>) -> Func {
    make_eval_step(
        &lang
            .coprocessors()
            .iter()
...
/// call_cproc(cproc_name, evaluated_args, env, cont): 3 {
///     let err: Cont::Error;
///     let nil = Symbol("nil");
///     let nil = cast(nil, Expr::Nil);
...
///     return (cproc_name, env, err);
/// }
fn call_cproc(cprocs: &[(&Symbol, usize)]) -> Func {
    if cprocs.is_empty() {
        func!(call_cproc(cproc_name, _evaluated_args, env, _cont): 3 => {
            let err: Cont::Error;
            return (cproc_name, env, err);
        })
    } else {
        ...
        Func::new("is_cproc".into(), func_inp, 3, Block { ops, ctrl }).unwrap()
    }
}
...
mod tests {
    ...
    fn test_dumb_lang() {
        ...
        let func = make_eval_step_from_lang(&lang);
