...
use fcomm::{
  public_params, Claim, Commitment, CommittedExpression,
  CommittedExpressionMap, Evaluation, Id, LurkPtr, NovaProofCache, Opening,
  Proof,
};
  ...
  fn new_from_path<P: AsRef<Path>>(p: P, base_prompt_length: usize) -> Self {
    ...
    for line in lines {
      ...
      if line.is_empty() {
        ...
        let input_line =
          if current_input.is_empty() { line } else { indent.clone() + &line };
...
impl ReplTrait<F> for ClutchState<F> {
  ...
  fn prompt(&mut self) -> String {
    if let Some(demo_input) =
      self.demo.as_ref().and_then(|d: &Demo| d.peek_input())
    {
  ...
  fn handle_non_meta(
    ...
    let (input, output, iterations) =
      self.repl_state.handle_non_meta(store, expr_ptr, update_env)?;
...
impl ClutchState<F> {
  ...
  fn chain(
    ...
    let new_secret = store
      ...
      .and_then(|hash| {
        store.get_expr_hash(&hash).ok_or_else(|| anyhow!("hash missing"))
      })
      .map(|hash| *hash.value())?;

    let (_, new_fun) =
      store.open(new_comm).ok_or_else(|| anyhow!("opening missing"))?;
    ...
    let interned_commitment =
      store.intern_maybe_opaque_comm(new_commitment.comm);
    let mut handle = io::stdout().lock();
  ...
  fn get_proof(
    ...
    self.proof_map.get(&cid).ok_or_else(|| anyhow!("proof not found: {cid}"))
  }
