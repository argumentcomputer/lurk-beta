...
mod test {
  ...
  fn outer_evaluate_recursion1() {
    // This test is an example of simple usage. Compare to the commented-out
    // original source for this test.
    ...
    let (
      ...
      Evaluator::new(expr, empty_sym_env(&s_), &mut s_, limit).eval().unwrap();
