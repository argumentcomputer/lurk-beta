...
use lurk_ff::tag::{ContTag, ExprTag, Tag, TagKind};
use std::fmt;
...
impl<F: LurkField> Ptr<F> {
    pub fn immediate(self) -> Option<Entry<F>> {
        match self.tag.kind {
            ...
            TagKind::Expr(ExprTag::Str) if self.val == F::zero() => Some(Entry::Expr(Expr::StrNil)),
            TagKind::Expr(ExprTag::Cons) if self.val == F::zero() => {
                Some(Entry::Expr(Expr::ConsNil))
            }
            TagKind::Expr(ExprTag::Sym) if self.val == F::zero() => Some(Entry::Expr(Expr::SymNil)),
    ...
    pub fn is_immediate(self) -> bool {
        self.immediate().is_some() || matches!(self.tag.kind, TagKind::Op1(_) | TagKind::Op2(_))
    }
}
