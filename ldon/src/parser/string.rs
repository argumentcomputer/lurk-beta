...
where E: ParseError<&'a str> + FromExternalError<&'a str, std::num::ParseIntError>
{
  ...
  let parse_delimited_hex = preceded(
    char('u'),
    // `delimited` is like `preceded`, but it parses both a prefix and a
    // suffix. It returns the result of the middle parser. In this case, it
    // parses {XXXX}, where XXXX is 1 to 6 hex numerals, and returns XXXX
...
{
  alt((
    // The `map` combinator runs a parser, then applies a function to the
