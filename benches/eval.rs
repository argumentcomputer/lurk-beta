...
fn criterion_benchmark(c: &mut Criterion) {
  ...
  c.bench_function("go_base_10_16_bls12", |b| {
    ...
    b.iter(|| {
      let result =
        Evaluator::new(ptr, empty_sym_env(&store), &mut store, limit).eval();
      black_box(result)
    })
  });
  ...
  c.bench_function("go_base_10_16_pasta_pallas", |b| {
    let mut store = Store::default();
    let ptr =
      go_base::<pasta_curves::Fp>(&mut store, black_box(10), black_box(16));
  ...
  c.bench_function("go_base_10_160_pasta_pallas", |b| {
    let mut store = Store::default();
    let ptr =
      go_base::<pasta_curves::Fp>(&mut store, black_box(10), black_box(160));
